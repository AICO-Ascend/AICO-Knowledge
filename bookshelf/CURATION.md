...
## 上架铁律（bookshelf_build.py 机械强制，违规拒绝生成）

这个书架是**给人类学习者的**，质量标准 = **一跳直达**：点开链接必须落在总结好的知识正文上。
新增知识源入库后，往书架加条目时必须过这四条闸门（G1-G4 在生成器里机械执行）：

- **G1 概念页不上书架**：`wiki/concepts/*` 是种子态（仅成员链接列表，且 wikilink 语法在网页端不渲染），
  只服务机器/Obsidian 层。要引用某概念，就引用它最好的那篇深读，不引概念页。
- **G2 骨架仓卡不上架**：`repocard:` 仅当存在分析层（`extraction/deep/repo-<slug>.md`）才可用。
  只有机械骨架的仓 → 记入文末「泊车场」；想让它立刻上架，改链仓内七节深读文档（`reponote:`）或仓外链（`ext:`）。
- **G3 摘要必须中文**：论文条目的摘要列由生成器从 `deep/<slug>.md`「核心问题」首段机械提取（≤3 句），
  无深读/无该段的论文禁止上架（先补深读，再上架）。禁止用英文 abstract 凑数。
- **G4 分类词表收敛**：`category` 只能取生成器 `CATEGORY_ORDER` 里登记的词（RL/Agent/架构/稀疏注意力/MoE/
  投机解码/多模态/综述/扩展/训练系统/训练框架/推理系统/推理框架/算子/系统软件/硬件）。
  新词先进词表再使用；框架类一律二分「训练框架/推理框架」，不准自立门户（MindIE 是产品名不是分类）。

配套约定（不强制机械校验，但策展时遵守）：

- **排序**：表内按分类聚合、类内按发表时间——由生成器保证，不要在 yaml 里手工排。
- **标题**：知识源显示名追求"不看正文也知道是什么"；仓条目不带中文括号注释（如 `MindIE-LLM（昇腾商用推理引擎）` ❌）。
- **摘要列**：🔥⚡ 符号 → 日期 → 内容摘要 → 备注/资产链接，语言通顺、统一中文（MLA/MoE 等专有名词除外）。
- **降级不删除**：条目下架一律进「泊车场」并注明原因，深读补齐后恢复——不静默删条目。
