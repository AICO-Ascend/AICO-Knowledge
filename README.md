...
- **669 张裁剪单图**（不再是整页截图——每张 figure 独立裁剪，可直接插入报告）
- **390 张裁剪表格**（表格第一次成为"可插入的图"，不再只是 Markdown 文本）
- **479 条 LaTeX 权威公式**（58 篇）+ **9 张公式截图**（无 LaTeX 源论文兜底，引用前核对）
- **1039 条 MiniMax-M3 多模态解读**（架构图/数据流图/表格逐张技术解读）
...
│   └── minimax_captions.json        #   多模态解读（900+ 条）
...
**现状（2026-08-23）**：69 唯一论文 ｜ 669 裁剪图 + 390 裁剪表 + 9 公式截图 ｜ 479 LaTeX 公式（58 篇）｜ 1039 MiniMax-M3 解读 ｜ 69 篇 6 段深读 ｜ PDF 0 截断 ｜ 三铁律全绿。
