...
> **v5 修订（2026-09-05，用户拍板）**：
> ① AscendInfra 主入口改为 **MD 版**（bookshelf/ascend_infra.md，HTML 旧版留作备份）——
>    gitcode 文件页对 .html 显示源码不渲染，MD 一跳直达；
> ② SHELF.md 摘要列论文一律用**深读「核心问题」首段的中文摘要**（≤3 句，禁英文 abstract）；
> ③ L2 模型/算法合并为单表（表内按知识分类聚合排序）；MindIE 独立分区解散并入推理；
> ④ 上架质量标准：一跳直达——概念页种子（仅链接列表）与骨架仓卡（无深读层）一律下架，
>    后者记录于 CURATION.md 末尾「泊车场」待深读补齐后恢复。
>
