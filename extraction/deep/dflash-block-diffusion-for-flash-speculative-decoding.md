...
- **Diffusion drafter 的两条既有死路**（§1, §2.3）：(i) 大型 dLLM drafter（DiffuSpec、SpecDiff-2，7B 参数）内存不可负担，起草延迟高，实际仅 3–4×；(ii) PARD 用小 AR 模型模拟 diffusion 式并行，但容量不足，加速天花板约 3×。同时独立 dLLM 本身质量劣于 AR（§1），且需要大量 denoising steps 才能保质量（§1, §2.2）。

DFlash 的核心命题：**能否同时做到轻量 + 高接受率 + 并行起草？** 关键 insight 是"the target knows best"——大 AR target 的 hidden features 隐式编码了未来多 token 信息（§1, §4.1，引用 Samragh et al. 2025）。DFlash 把 draft model 重新定位为 **diffusion adapter**，直接复用 target 已建模的深层上下文，而非让小模型从零推理。

## 关键创新点

1. **KV injection 条件化机制**（§4.1, §A.3）
...
2. **Block-diffusion 并行起草**（§3.2 eq. 3, §4.1）
   - **机制**：所有 γ 个 masked token 在 **单次 forward pass** 内并行解码，`T_draft = t_parallel`，与 γ **几乎无关**。GPU 对并行操作效率远高于多次串行 pass，`t_parallel ≪ γ · t_step`。
   - **效果**：打破 AR drafter 的"浅模型-低 τ"瓶颈，可负担 5 层（Qwen3-Coder 用 8 层）更深、更具表达力的 drafter。Figure 3：5 层 DFlash 生成 16 token 的延迟 **低于** 1 层 EAGLE-3 生成 8 token。Pareto 前移：5 层 DFlash @ block 16 同时获得更低 latency + 更高 acceptance length。

3. **Speculative-decoding-tailored 训练**（§4.2）
   ...
   - **Exponential loss decay**（eq. 4）：`w_k = exp(−(k−1)/α)`，α 对 block size 16/10/8 分别为 7/5/4（§A.1）。理由：speculative decoding 中 block 早期位置错误会 **invalidates all subsequent tokens**，早期预测对 τ 不成比例地重要。Figure 5：loss decay 使训练更快收敛且最终 τ 更高。
...
4. **Efficient long-context training**（§4.2）
   ...
   - **效果**：base 4K 训练的 drafter 仅用 1.6K LongAlign 样本 fine-tune 3 epoch 即可适配长上下文。Table 4（§5.4）：Base 在 16K/32K hotpotqa 上 τ 跌至 3.61/–，Long 版升至 6.05；gov_report 32K 从 2.09 升至 3.56。
...
**关键数字**：T=0 平均加速 4.9×，比 EAGLE-3(16) 提升 2.4×；T=1 平均 4.1×，比 EAGLE-3(16) 提升 2.2×。DFlash(16) 的 τ 在多数任务上甚至超过 EAGLE-3(60)，且 verification 开销更低。
...
注意高并发 32 时 EAGLE-3(60) speedup 降至 0.6×（负加速），DFlash 仍保 1.6×–1.8×。
...
- **Block diffusion 锚点**：[[block-diffusion-interpolating-between-autoregressive-and-diffusion-language-models]] —— DFlash 的技术基座；DFlash 把 block diffusion 从独立生成模型重构为 speculative decoding 的 drafting adapter，并用 KV injection + random anchor sampling + loss decay 改造训练目标对齐推理行为。
...
- **EAGLE 家族**（DFlash 的主要对照基线）：
  - [[eagle-speculative-sampling-requires-rethinking-feature-uncertainty]] —— feature-level context 范式奠基，DFlash 继承"借力 target hidden feature"思想但改 input fusion 为 KV injection。
  - [[eagle-2-faster-inference-of-language-models-with-dynamic-draft-trees]] —— 动态 draft tree；DFlash 用 block-diffusion 单 pass 并行替代 tree drafting。
  - [[eagle-3-scaling-up-inference-acceleration-of-large-language-models-via-training-time-test]] —— DFlash 的 head-to-head 对照（§5.1, §5.5.1），2.2–2.5× 优势；EAGLE-3 的 training-time test 在长上下文上成本高，DFlash 用固定 block 数 + random anchor 规避。
...
- **MT-Bench/chat 任务加速偏低**：Table 1 中 MT-Bench speedup 仅 2.75×（Q3-8B）/2.85×（Q3-4B），远低于 Math 任务的 5–6×。chat 任务的 τ 也最低（4.24–4.35）。说明 block-diffusion drafting 在自由对话、低结构化输出上优势收窄。
...
- **Denoising step 数未深入消融**：论文强调"aggressive reduction in denoising steps"（§6）但正文未给出 step 数与质量/延迟的曲线，仅 Figure 3 给出 draft latency 对比。
