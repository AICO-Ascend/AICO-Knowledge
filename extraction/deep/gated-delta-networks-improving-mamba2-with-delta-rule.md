# Gated DeltaNet — 技术点深读（DEEP 2026-08-18）
> 全要素深读笔记。独立文件，extract_phase1 重跑不丢。
> 论文：Gated Delta Networks: Improving Mamba2 with Delta Rule · arXiv:2412.06464 (v3, 2025-03-06, ICLR 2025)
...
论文核心动机：上述两机制是 **互补的**——gating 擅长 rapid memory erasure，delta rule 擅长 targeted update。二者合流即得本文 gated delta rule。
...
1. **Gated delta rule（Eq.10, §3.1）** —— 核心机制
   ...
   中间值则同时实现"遗忘 + 定向写入"。结构上即把 Mamba2 的对角转移 αt I 升级为 generalized Householder 矩阵 αt(I − βt kt kt^T)，兼顾遗忘幅度与方向性。
...
5. **硬件高效 chunkwise 训练算法（§3.3, Appendix A）** —— 工程核心贡献
   ...
   - 关键工程结论（§4 throughput）：gated delta rule 相对纯 delta rule **只引入边际开销**，GDN 与 DeltaNet 训练吞吐基本一致；二者仅比 Mamba2 慢 2–3K t/s（因转移矩阵更具表达力）。

6. **Block 设计与混合架构（§3.4, Fig.1）**
   - Token mixer block：沿用 Llama 宏观结构（mixer + SwiGLU MLP 堆叠），self-attention 替换为 gated delta rule。q/k 路径 = linear proj + short conv + SiLU + L2 norm；v 路径 = linear proj + short conv + SiLU；α、β 仅 linear proj；输出 norm + gating（SiLU）+ output proj（Fig.1 右）。α 复用 Mamba2 的参数化（§3.4 脚注 4）。
   - **H1 = GDN + SWA**（类 Griffin/Samba）；**H2 = Mamba2 + GDN + SWA**（三路交错）。Fig.3 示 H1/H2 吞吐优于 standalone GDN，H1 在所有序列长度上保持高吞吐。
...
- **vs Mamba2**（§1, §3.2, Table 3/5）：Mamba2 用对角转移 αt I，均匀衰减；GDN 用 αt(I − βt kt kt^T) 兼具遗忘幅度与方向性。S-NIAH-1 长序列上 GDN 8K 91.8 vs Mamba2 30.4；Wiki ppl 16.42 vs 16.56；LongBench avg 16.6 vs 13.5。Mamba2 在 S-NIAH-3 UUID 上崩塌（4K 4.6）而 GDN 保持 27.6。
- **vs DeltaNet**（§1, §3.2, Table 2/4）：DeltaNet 无 forget gate，真实世界 retrieval（Table 4 avg 26.2）与 S-NIAH-2/3 长序列（4K 18.6/22.4）显著掉点；GDN 在两者上均升级（30.6 / 92.2 / 27.6）。但纯 synthetic S-NIAH-1 DeltaNet 接近满分，说明 retention 任务无需 gating。
...
- **吞吐对比**（Fig.3, §4）：Transformer++ 在 2K 短上下文因 Flash-Attention-2 最快；GDN ≈ DeltaNet，比 Mamba2 慢 2–3K t/s（更 expressive 的转移矩阵所致）；混合模型 H1/H2 因 SWA 局部并行而吞吐高于 standalone GDN。
...
- **[[samba-simple-hybrid-state-space-models]]** & **[[griffin-mixing-gated-linear-recurrences-with-local-attention]]** — 混合架构前作；GDN-H1/H2 直接对标其 linear+SWA 范式。
...
2. **Length extrapolation 结果混合**（§4, Fig.2）：GDN 在六项 long-context 任务上整体 ppl 最低，但并非一致领先；hybrid 模型靠 SWA 处理局部上下文才进一步缓解——说明 GDN 的记忆管理在 >20K 序列上仍有压力，作者明示"future work will explore…even longer sequences"。
...
7. **无大规模实验**：主实验仅 1.3B / 100B tokens，scaling 行为未验证；ablation 在 400M / 15B tokens 上完成。
