...
2. **三矩阵乘的 ScanUL1（Algorithm 4.2, §4.1）——把单 tile 内 scan 完全用 cube 表达。**
   基于 [13, Alg.6] 的恒等式（Eq. 4.1，formulas.json [0]）——LaTeX 权威渲染：
   $$ \texttt{scan}(\z) = \matA_s\ @\ \matU_s + \matL^{-}_s\ @\ \matA_s\ @ \ \ones_s, $$
   其中 `A_s` 是把长度 `ℓ=s²` 的 tile row-major reshape 成的 `s×s` 矩阵，`U_s` 含主对角线的全 1 上三角方阵（行内局部 s-长前缀和），`L_s⁻` 严格下三角全 1（跨行传播），`1_s` 全 1 方阵。三步矩阵乘序列（formulas.json [1]，LaTeX 权威渲染）：
   $$ \begin{aligned} \matC_1 &= \matA_s\ @\ \ones_s \nonumber \\ \matC_2 &= \matA_s\ @\ \matU_s \nonumber \\ \matC_2 &= \matC_2 + \matL_s^{-}\ @\ \matC_1. \nonumber \end{aligned} $$
   **LaTeX↔M3 双源校验**：M3 p03/p04（Fig.3.1）解读"AIC 输出经 L0C/FixPipe 回 Global Memory，AIV 从 GM 取到 Unified Buffer，Cube↔Vector 无本地 UB 通路"——与本式数据流一致：三步矩阵乘在 AIC 的 L0A/L0B/L0C 内完成，结果写回 GM，再由 AIV 在 UB 内做 `partial` 传播；式 (3) 复用 `C2` 正对应 L0C accumulation buffer。两个收益点：(a) 前两步共享左操作数 `A_s`，`A_s` 只需载 L0A 一次；(b) 第三步利用 Cube 的 accumulation buffer（L0C）复用 `C2`。矩阵乘次数 = `3⌈n/s²⌉`，但 vector span 降到 `O(n/s²)`（vector length 为 `O(s²)`），相对 ScanU 有 `O(s)` depth 优势。实测 `s=128` 时 ScanUL1 ≈ 1.92× 快于 ScanU（§4.1 末，对照 Figure 4.2 的 log-log 曲线），与理论上界同量级。单核相对 vector-only CumSum 5×（ScanU）~ 9.6×（ScanUL1）（§1, §4.1）。
