...
> M3 figure caption 来源：extraction/minimax_captions.json（5 张：Figure 4 p8 / Figure 11 p16 / Figure 17 p22，及 p12、p23 文本页无图，仅引用 Fig 8/9/10、Fig 18/19）。
...
3. **Disaggregation 缺失**：prefill（compute-bound）与 decode（memory-bound）争用资源；MoE（stateless，随 batch size scale）与 attention（stateful KV cache，随 seq len scale）资源不对称，colocated 部署造成资源浪费与互相干扰。Figure 17（p22，M3 解读）即给出这一动机的可视化：Job Executor 将 M 个 prefill TE 与 N 个 decode TE 全互联，prefill DP group 用 TP=4（compute-bound），decode DP group 用 TP=1（memory-bound），同 DP group 内做长度同质化聚簇以避免尾延迟——图证解耦的必要性。
...
1. **Transformerless 解耦执行架构**（§1、§5）。将 transformer 拆为 attention / feedforward / MoE 独立模块，跑在专用 NPU 上。三阶段演进（Figure 16）：(a) Disaggregated Prefill-Decode（§5.1，Figure 17 p22），(b) Disaggregated MoE-Attention（§5.2，Figure 18/19 p23 引用但未渲染），(c) Vision: dataflow serving（去全局同步，§5.3）。机制：prefill 用 TP=4 + eager graph + sequence parallel + prefix cache；decode 用 TP=1 + static graph。DeepSeek-V3/R1 部署在 768 die：288 die 跑 EP288（256 routed + 32 shared experts），480 die 跑 MLA（§5.2）。Figure 17（M3）清晰展示 9 步工作流：Job Executor 分配→prefill DP 调度→RTC-DistFlow 注册 KV→JE 派发到 decode TE→decode DP 执行→延迟 KV 迁移→完成，强调 prefill/decode 各需独立 DP grouping 且按长度同质聚簇。

2. **XCCL 内存语义通信库**（§3）。基于 CloudMatrix384 global shared memory 构造 far-memory 风格原语，非传统 network verbs（对照 DeepEP [2]）。
   - **P2P send/recv**（§3.1，Figure 4 p8）：每 NPU on-chip memory 三分区——app data area（KV cache）、metadata area（eventID/chunkID/tailPtr，32B/field）、managed data area（ring buffer，head/tail）。总 fields 数 = 384×2×48×2 ≈ 74K，metadata 总 4 MB。**Figure 4（M3 解读）**给出 8 步协议：①Sender AIV 经 MTE2 读 app→unified buffer；②ping-pong unified buffer 经 MTE3 写到 receiver on-chip memory（与①并行，藏延迟）；③Sender 更新 receiver metadata；④Sender poll；⑤Receiver kernel 校验 metadata 匹配；⑥Receiver 拷回 app area；⑦Receiver 经 metadata 通知 sender；⑧Sender 返回 CPU。M3 要点：ping-pong unified buffer 使 MTE2/MTE3 并行藏延迟；另有 DMA-engine 零拷贝变体直接操作 app data area，启动延迟更高但省 unified buffer。性能：<1 MB 负载 <20 μs（2 AIV cores 即可）；9 MB 用满 48 AIV cores 比 2 cores 快 >2.5×。可扩展到 ~300K NPU pair。
   ...
   - **A2E/E2A**（§3.3，Figure 8 p12 引用但 M3 该页未渲染）：disaggregated MoE-Attention 专用，处理 attention NPU 与 expert NPU 数量不对称（如 288 expert vs 160 attention）。

3. **Trampoline Forward 机制**（§3.3、§5.2，Figure 8 p12 引用）。针对 attention/expert NPU 不对称：选与 attention NPU 数量相等的 expert NPU 作 trampoline，先收全部数据再转发给其余 expert NPU。两段式路由（A2E→A2E'、E2A→E2A'，Figure 19 p23 引用）降低 metadata fan-out 与带宽不平衡。实测（§3.3）：3 DP domain × 160 DP group（TP=1）+ 288 expert NPU，per-die batch 96 → global batch 46,080，A2E=172 μs，E2A=193 μs。

4. **NPU-Direct URMA**（§3.3）。类 GPU IBGDA：AIV core 直接向 DMA engine 发 remote memory 请求，绕过 host CPU 与 AI CPU（与 Figure 4 M3 提到的零拷贝 DMA 变体呼应）。虽启动延迟高于 MTE2/MTE3，但三优势：(1) 释放 AIV 资源；(2) 适合高吞吐（DMA 支持数 GB，MTE 受 unified buffer 数百 KB 限制）；(3) 避免与 compute stream 抢 MTE2。

5. **FlowServe 去中心化 DP group 抽象**（§4.1、§4.2，Figure 9/10 p12 引用但 M3 该页未渲染）。受 SGLang [24] 启发。每 DP group 自含完整 pipeline（tokenizer、API parser、SPMD executor、RTC、DistFlow），跨 DP 无通信。单 FlowServe 实例可跨整 SuperPod（48 server / 768 die）。TE-shell 仅保留三职责：跨 DP 请求分发（§4.3）、触发 expert LB（§4.5）、健康检查协调（§6.1）。**Output shortcutting**：每 DP master fork 子进程专做 detokenization + 流式解析，直连前端，避免中心化输出瓶颈。Figure 10（§4.2 引用）展示 unbalanced KV usage 跨 group 导致 MLA 执行时间不均，是 DP-LB 与 EPLB 的动机图证。
...
### 表2：XCCL 三类原语（§3，Figure 4/8）
| 原语 | 用途 | 数据结构 | 关键性能 |
|---|---|---|---|
| send/recv（Fig 4 p8） | disaggregated PD KV 迁移、sequence parallel、npu-fork | 74K metadata field × 32B，4 MB metadata；per-pair ring buffer；ping-pong unified buffer；零拷贝 DMA 变体 | <1 MB <20 μs（2 AIV）；9 MB 用 48 cores 快 >2.5× |
...
| A2E/E2A（Fig 8 p12） | disaggregated MoE-Attention | 同 dispatch + trampoline 两段路由（Fig 19） | global 46,080 batch：A2E 172 μs，E2A 193 μs |
...
- **vs DeepEP [2]**（§3.2）：DeepEP 用传统 network-level verbs；XCCL 用 far-memory 语义 over global shared memory（Figure 4 p8 M3 可见的 metadata + ring buffer + ping-pong 协议），更贴合 UB fabric。
...
- → [[mooncake-a-kvcache-centric-disaggregated-architecture-for-llm-serving]]：同为 disaggregated PD 谱系；Mooncake 是 GPU/KVCache-centric 代表，本文是 Ascend/SuperPod 内存语义代表，对照两种 disagg 路线。Figure 17（p22 M3）的 prefill/decode TE 全互联拓扑正对应 Mooncake 的 disagg KV 迁移范式。
...
- → [[ascend-950-npu-architecture-whitepaper]]：910C 是 950 的前代，本文提供 910C 在 SuperPod serving 中的实测行为（MTE2/MTE3、DMA、AIV/AIC、URMA，Figure 4 p8 M3 可见的 on-chip memory 三分区与 ping-pong），为 950 架构演进提供生产基线。
- → [[a-survey-on-large-language-model-acceleration-based-on-kv-cache-management]]：本文 KV cache 迁移（P2P，Figure 4）、KV INT8 量化、non-RoPE 量化是 survey 中 disagg + quant 方向的工业实例。
- 谱系定位：**serving-system / NPU-cluster 主题**，Ascend 系（vs Mooncake/SGLang 的 GPU 系）；disaggregation 三阶演进的工业落地。
- 关联参考（论文内引用，非库内）：DeepEP [2]、DaVinci [14]、LegoOS [19]（disaggregation 原则）、FaRM [3]（far-memory 协议原型，Figure 4 协议的远源）、DeepSeek [15]、Kimi K2 [12]、Qwen3 [23]、GLM-5 [5]。
...
- **dispatch/combine 方差大**（§7.1，Figure 20）：max 可达 min 的 10×（dispatch 185→1231 μs；combine 165→2939 μs），源于吸收 MLA 与 MoE 跨 NPU/expert 不均，仍是尾延迟风险。
- **EPLB 周期性、非实时**（§4.5，Figure 11）：每分钟采样一次，对突发负载倾斜响应滞后；redundancy slot 预算 R 有限，极端 hot expert 可能仍不均。M3 读图与文本 >40% 改善口径存在 ~10% 视读差异，定量以文本为准但需独立复测。
- **metadata 固定 4 MB、非零拷贝**（§3.1，Figure 4 M3）：当前实现数据需在 app data area 与 managed data area 间拷贝（非零拷贝版本另存但未默认）。
...
- **异构 prefill 跨 fabric**（§5.1，Figure 2）：910B→910C 跨 RoCE/VPC 传 KV，SLA 满足但带宽低于 UB，扩展受 fabric 限制。
- **可靠性评估缺失**（§6）：仅给机制，未给 MTTR/可用率数值或故障注入实验。
- **测评模型单一**（§7）：实测以 DeepSeek-R1/V3 为主；Kimi K2 / Qwen / GLM / MiniMax 仅声明支持，未给同口径 benchmark。
- **M3 figure 覆盖不全**：5 张候选中 p12、p23 为纯文本页，Figure 8/9/10/18/19/20 仅有论文内引用而无 M3 视觉 caption，trampoline 两段路由、FlowServe 架构、MoE-Attention pipeline、延迟分解图未获 M3 图证，相关解读依据文本。
