# Dynamic Large Concept Models — 技术点深读（DEEP 2026-08-18 重写）
...
**机制**：把 NTP 拆成可解耦的四级管线（与 Figure 1 M3 caption 一致）：
```
...
**机制**：
- **Boundary Detection（§3.3.1, Eq. 5–6）**：把 token 投影到 `d_scan` 维 query/key 空间，边界概率 = 相邻 token 的归一化余弦不相似度：`p_t = (1 − cos(q_{t−1}, k_t))/2`，并强制 `p_1 = 1`（首 token 必为边界）。训练期用温度 α 锐化后 `Bernoulli(p_sharp)` 采样以鼓励探索；推理期硬阈值 `b_t = [p_t ≥ 0.5]`。这正是 Figure 1(b) M3 所示 `sim(h_t, h_{t−1}) < τ → boundary`。
- **Concept Formation（§3.3.2, Eq. 7）**：按边界把 token 分成连续段 `S_1..S_M`，段内 mean pooling 后用 `W_up∈R^{d_concept×d_token}` 升维到概念空间。
- **Adaptive Compression via Global Load Balancing（§3.3.3, Eq. 8–10）**：目标压缩比 R（R=4 表示平均 4 token/concept）。统计不按序列而按**全局分布式 batch**：`G_global`（期望边界率, Eq.8）、`F_global`（实际边界率, Eq.9），AllReduce 同步，辅助损失
  `L_aux = R/(R−1)·[(R−1)·F·G + (1−F)·(1−G)] − 1`（Eq.10）。
...
**机制**：解码端要把概念级表示 `Z̃`（经 §3.5.1 Concept Smoothing Eq.11 消除硬池化离散伪影）映射回 token 级。因 `d_token ≠ d_concept`，cross-attention 把 query（来自 encoder）和 K/V（来自 concept）投到共同 head 维 `d_head`（Eq. 12–13）。**因果性约束**（§3.5.2）：token t 只能 attend 到 index ≤ `j(t) = Σ_{i≤t} b_i` 的概念，防止未来概念泄漏——Figure 1(c) 的 Causal Mask 即此。

实现难点（§4.1, Figure 2）：变长 token→concept 映射产生不规则 L×M 注意力 mask，Flex Attention 生成动态 mask 与不规则内存访问开销大。**Concept Replication**（Eq. 17）：用 `repeat_interleave` 把概念 K/V 按段长复制到 L 长，把 L×M 不规则问题变成 L×L 标准因果 self-attention（K/V 段内恒定），直接用 Flash Attention Varlen + 高度优化的 CUDA causal kernel。Figure 2（M3 caption, p07）清晰对照了左图的 "Irregular Mask (L×M) / Flex Attention" 与右图的 "Regular Alignment / Standard Causal Mask (L×L) / Flash Attention"——把概念 `c1,c2,c3` 复制成 `c1,c2,c2,c3,c3` 对齐 token query。
...
**机制**：扩展 Chinchilla 框架，把损失分解为 token 处理效率、concept 处理效率、数据 scaling 三项（Eq. 22）：
```
L(N,D,R,P) = E0 + A_token / (N(1−P)+t_token)^δ1
           + A_concept / (R·(N·P+t_concept))^δ2     (Eq. 22)
           + A_data / (D+t_data)^γ
```
其中 N 总参、D 数据、R 压缩比、P concept-backbone 参数占比；token 分支与 concept 分支分别配指数 `δ1, δ2`，concept 项把 R 置入分母底数（R 越大该分支 loss 贡献越小，即压缩越狠 concept-backbone 的每参数有效容量越大）。注意 §6.2.2 文中 "controlled by exponent γ" 与公式中 γ 实际控制 data 项、δ2 控制 concept 项存在表述错位，以公式为准。

针对 WSD（Weight-Sharing-and-Decay）训练协议的 late-stage，单独拟合 decay 相（90%–99% token 窗口，Eq. 23）：`Δ_decay = k·L_stable^a · R^b · N^c`，log-linear 回归 **R² = 0.93**（Figure 5）。

**效果（§6.2.1, §6.4, Figure 4/5）**：实验网格 `P∈{30%,50%,70%} × R∈{2,4,8}`，200B token，Small/Medium/Large = 274M/468M/833M；所有 scale 与 R 共享同一组 `(δ1, δ2, γ)`，仅 scale-independent offset 不同——避免后验过拟合（§6.2.1）。全轨迹联合拟合 **R²>0.98**（Figure 4）；tail-focused sampling 100B 轨迹外推到 1T 验证，拟合误差 <0.05。预测**有效 compute multiplier ≈ 1.4**，与标准 baseline factor 1.34 接近，互证可信。这是首个把 token 容量、concept 推理容量、压缩比**显式解耦**的 scaling law。
...
- 粒度来源：LCM 用**冻结 SONAR encoder/decoder** 把句子映射到固定语义空间，句子边界是预定义人类先验；DLCM 边界**从表示空间端到端学到**，变长、内容自适应（Figure 1(b) 判据）。
...
- DLCM 把 H-NET 的"学到的边界 + 自适应计算"原则**移植到 token 级 decoder-only LLM**（Figure 1），并以 Global Parser（全局正则，Eq. 8–10）替换 H-NET 的纯可微边界，避免 CE 损失压过压缩目标（见 §8.1, Figure 8）。
...
1. **离散边界与 LM loss 显式解耦（§3.3, §8.1, Figure 8）**：论文刻意把离散分段决策从 LM loss 中解耦以保训练稳定。代价是分段**不是真正端到端**由 LM loss 优化的——Figure 8 的 ablation 显示纯端到端可微边界预测器（learned predictor，红线）会 "creep-up"：压缩长度从 ~2000 退化到 ~4300（1.9× 而非 4×），因为 CE 梯度（anti-compression）远大于 `λ‖∇L_aux‖`（pro-compression）（Eq. 24）。最终采用 rule-based 阈值版（`p_t = (1−cos)/2`，阈值 τ）以保稳定——即"学表示，但分段规则不学"。这削弱了"从数据学到任务最优分段"的原始主张。
...
5. **全局负载均衡的工程假设**：Global Parser 依赖 AllReduce 同步 `G_global, F_global`（§3.3.3, Eq. 8–9），且 VarLen packed sequence training 要求压缩统计跨多样 token（§4）。在大规模分布式训练下，这要求 batch 内内容多样性足够；若 batch 偏单域（如纯 code），全局 R 可能漂移。论文未给大规模分布式下的鲁棒性数据。
...
7. **可解释性有限**：概念虽名义上"可解码"，但 mean-pooled 后的 `c_k` 与具体 token 序列是多对一映射；concept smoothing（§3.5.1）进一步混合相邻概念。论文未给出概念级可解释性的量化评估，与 LCM 那种"每个概念 = 一个可解码句子"的可解释性相比偏弱。Appendix A（Figure 9 之前的 segmentation 示例）只给定性样例。
