...
Hyper-Connections（HC, Zhu et al. 2024）通过把 residual stream 宽度从 `C` 扩展到 `nC`（expansion rate `n`）并引入三个可学习线性映射 `Hres_l ∈ R^{n×n}`、`Hpre_l, Hpost_l ∈ R^{1×n}`，在不增加单层 FLOPs 的前提下显著提升拓扑复杂度与性能（§1, Eq.3）。但作者指出 HC 在大规模训练下暴露出两个机制级根因问题：

1. **数值不稳定（Numerical Instability, §3.1）**：HC 递归地跨层展开后，浅层 `l` 到深层 `L` 的有效信号传播由复合映射 `∏_{i=1}^{L-l} Hres_{L-i}` 控制（Eq.4）。由于 `Hres_l` **无任何约束**，该复合映射偏离 identity mapping 的"信号守恒"性质，导致前向/反向信号范数无界放大或衰减。实证证据（§3.1, Fig.2/3，27B 模型）：
...
**机制**：约束 `Hres_l ∈ Mres`，其中 `Mres` 是 doubly stochastic matrix 流形（Birkhoff polytope）——非负且行和、列和均为 1（Eq.6）。当 `n=1` 时退化为标量 1，即恢复原始 identity mapping。

这一约束带来三条严格理论性质（§4.1）：
- **Norm Preservation**：doubly stochastic 矩阵的 spectral norm `‖Hres_l‖_2 ≤ 1`，即 non-expansive，从机制上抑制梯度爆炸。
- **Compositional Closure**：doubly stochastic 矩阵集合在矩阵乘法下闭合，因此复合映射 `∏ Hres_{L-i}` 仍是 doubly stochastic——任意深度上都保持守恒，这是 HC 最缺的性质。
- **Geometric Interpretation**：Birkhoff polytope 是所有 permutation matrix 的凸包，故 `Hres_l` 是置换的凸组合，反复作用单调增强跨流混合，充当稳健的特征融合。

此外对 `Hpre_l`、`Hpost_l` 施加非负约束（§4.1 末），防止正负系数叠加引起的信号对消。
...
**机制**：mHC 沿用 HC 的 dynamic + static 双部分参数化（Eq.7），但把输入 `x_l ∈ R^{n×C}` **flatten** 为 `x̃_l = vec(x_l) ∈ R^{1×nC}` 保留全部上下文（不同于 HC 对每行独立做 RMSNorm）。线性投影参数变为 `φ_pre, φ_post ∈ R^{nC×n}`、`φ_res ∈ R^{nC×n²}`。最终约束映射通过（Eq.8）：
- `Hpre_l = σ(H̃_pre_l)`（Sigmoid → 非负）
- `Hpost_l = 2σ(H̃_post_l)`（Sigmoid×2 → 非负且均值为 1，匹配 residual merge 量纲）
- `Hres_l = Sinkhorn-Knopp(H̃_res_l)`

Sinkhorn-Knopp 算子：先 `exp(·)` 使所有元素为正，再交替做行归一化 `T_r` 与列归一化 `T_c`（Eq.9），迭代 `t_max` 次收敛到 doubly stochastic。**实验取 `t_max = 20`**（§4.2）。
...
- **Selective Recomputing（§4.3.2）**：丢弃 mHC kernel 的中间激活，反向时只重跑 mHC kernel（不重跑重的层函数 `F`）。对连续 `L_r` 层只存首层输入 `x_{l0}`。最优化目标给出最优 block size（Eq.20）：`L_r* ≈ sqrt(nL/(n+2))`。理论最优值恰好与 pipeline stage 的层数对齐，故**把 recomputation 边界与 pipeline stage 对齐**。
