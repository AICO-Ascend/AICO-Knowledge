# RLLM — 技术点深读（DEEP 2026-08-18，重写）

> 原文：rLLM: Relational Table Learning with LLMs (arXiv:2407.20157v2, 12 Nov 2025)。Shanghai Jiao Tong University + Tsinghua University。本文是一篇 **系统/框架型论文**（PyTorch 库 rLLM/relationLLM），附带 BRIDGE 示例算法与 SJTUTables 数据集，并非提出新的 LLM 训练范式。

## 核心问题

论文攻击的不是"模型能力不足"，而是 **"将 LLM 应用于真实关系型大数据的成本与工程不可行性"**，以及由此衍生的 **RTL（Relational Table Learning）方法学碎片化**问题。

- **成本不可持续性（§1 + Figure 1）**：Figure 1 是双面板复合图（M3 caption）。左面板为 2010–2025 全球数据量堆叠柱状图（Video/Image/Audio/Text/Structured data，从 ~30 ZB 升至 ~100+ ZB，Video/Image 主导体量）；右面板为同期 LLM token 成本堆叠面积图，**Text 与 Structured data 占据成本主体、呈指数攀升至 ~$5,000 trillion**，而 Video/Image 尽管体量大却只占很小的成本切片。M3 要点直接对应该图核心 takeaway——"LLM token costs are dominated by language-type and structured data, not multimedia"。作者据此给出量化锚点：2025 年 LLM 总成本近 **$5,000 trillion**，约为"2023 年美国 GDP（$27.37 trillion）的 214 倍"（§1 原文）；关系型数据库托管约 **73%** 的世界数据 [6]。
- **token 成本分布失衡（§1 + Figure 1 右图 + §A.1）**：文本与结构化数据体量小于多媒体，但因 UTF-8 编码（每字符 1 字节，英文词约 6–11 字符 [26]）+ OpenAI token 经验公式 [27]，其 token 成本占比反而最高；多媒体单位 token 成本相对低（图像 256×256 RGB 按 ViT [28] 切 patch + OpenAI 定价 [29]；视频按 Gemini 1.5 [30] 每秒一帧抽样）。Figure 1 右图的堆叠结构与该估算一致：**直接把关系表文本化喂 LLM 在经济上不成立**。
- **方法学碎片化（§2）**：GNN、LLM、TNN（Table Neural Network）三类工具各自发展，缺少统一的模块化分解，研究者无法快速组合它们构建 RTL 模型。
...
1. **三层架构的模块化分解（§2 + Figure 2/3）**
   - **机制**：Figure 2（M3 caption）描绘 rLLM 的 **三层层级架构**，向上箭头表示数据/控制递进——底层 Data Engine Layer（蓝色：`Data Loader`/`Graph Builder`/`Table Marker`，负责基础摄入）；中层 Module Layer（绿色：GNN 的 `GraphConv`/`GraphTransform`，LLM 的 `Prediction`/`Enhancement`，TNN 的 `TableConv`/`TableTransform`，即可复用神经原语）；顶层 Models Layer（橙色：`Combine`/`Align`/`Co-Train` 等组合算法）。M3 要点指出其核心在于 *data ingestion*、*modular neural primitives*、*compositional model algorithms* 的严格分离，使用户可 mix-and-match 图/语言/表格 building block 组装新 RTL 模型而无需重写数据流水线。Figure 3 进一步给出底层数据结构：`Dataset`（PyTorch）子类负责加载，`BaseGraph`/`BaseTable`（ABC）负责存储，二者解耦以追求 flexibility & scalability（§2.1），支持同构/异构图与表数据。
   - **效果**：任一 RTL 模型可由标准化模块组合，论文将 GCN/GAT/RECT/TAPE/OGC（同构）、HAN/HGT（异构）、TabTransformer/TabNet/FT-Transformer（单表）统一封装进同一接口（§4.1）。
...
2. **"Combine / Align / Co-Train"三策略范式（§2.3）**
   ...
   - **效果**：三策略可独立或组合使用，"rapidly develop various RTL-type models"（§2.3 末），为后续 RTL 方法提供构造模板。这三策略恰好对应 Figure 2 顶层 Models Layer 列出的 `Combine`/`Align`/`Co-Train` 三块（M3 caption 所述）。

3. **BRIDGE 示例算法（§3 + Figure 4）**
   - **机制**：Figure 4（M3 caption）描绘 BRIDGE 的 **两阶段编码流水线**——输入为多张经 PK–FK 连接的关系表（Table I/II/III…）；**Table Encoder** 消费原始表，提取 "Tabular features" 并投影为 "Tabular embeddings"，处理每张表内异构列类型；**Graph Encoder** 同时接收 tabular embeddings 与 **non-tabular features**（即 FK 构成的显式图结构 + 其它辅助特征），输出联合表示。M3 要点明确 BRIDGE 的三步简化：(a) 把多表输入降为"目标表 + 单张选定关系邻居表"；(b) 其余表全部预编码为 dense embeddings；(c) 用 GNN 建模残余 FK 结构。这把表内特征学习（Table Encoder，TableTransform/TableConv）与关系图推理（Graph Encoder，GraphTransform/GraphConv）解耦，使 intra-table heterogeneity 与 inter-table dependencies 可并发利用。训练目标可监督（cross-entropy）或无监督（基于表/外键的数据重构，§3 末）。
   - **效果（§5.3 + Table 2）**：在 TML1M 上 BRIDGE 用 TabTransformer 作 table encoder + GCN 作 graph encoder，准确率 **0.362±0.03**，相比单一 TabTransformer（0.347±0.02）/ FT-Transformer（0.352±0.02）有提升但幅度有限；在 TLF2K 上 **0.422±0.03**（vs TabTransformer 0.137±0.08，提升约 3 倍）；在 TACM12K 上 **0.256±0.01**（vs TabNet 0.135±0.01，约 1.9 倍）。关系表信息越丰富（TLF2K、TACM12K 含多张关系表/多关系）BRIDGE 优势越显著——这与 Figure 4 中 Graph Encoder 跨表聚合的设计意图一致。
...
- **vs RDL [9]（Relational Deep Learning, Fey et al.）**：BRIDGE 是 RDL 的简化版——RDL 把整个关系数据库的多张表与外键全部建模为异构图；BRIDGE 仅保留 **单张关系表**，其余表预编码为 dense embedding 后注入（Figure 4 M3 要点 a/b/c 的三步简化）。代价是损失多跳关系建模能力，收益是架构复杂度大幅降低，便于教学/快速实验。RelBench 数据集 [9] 与 SJTUTables 互补：RelBench 贴近商业（user lifetime value、论坛活跃度预测）但噪声高、不平衡；SJTUTables 是经典数据集增强版，干净、平衡、固定划分，适合方法原型期对照（§4.2 末作者建议先用 SJTUTables 再去 RelBench 评估）。
...
- **vs 单表 TNN（TabTransformer/TabNet/FT-Transformer）**：单表 TNN 只学目标表内部列间关系，忽视跨表外键信息，在 TLF2K/TACM12K 上准确率塌陷（TabTransformer 仅 0.137/0.091）。BRIDGE 通过 Figure 4 的 Graph Encoder 引入跨表关系，在 TLF2K 上提升约 3 倍，证明关系表建模的边际价值。
...
- **vs LLM 直接推理范式**：论文核心立场是 **避免把关系表文本化喂 LLM**（成本不可持续，§1 + Figure 1 右图），改为用 LLM 做 Predictor（标注）/ Enhancer（增广）这类局部工作，主体表征学习交给 TNN+GNN。这与"用 LLM 端到端处理一切"的路线形成方法论分歧。
...
- **BRIDGE 建模能力有损**：BRIDGE 只保留单张关系表（Figure 4 M3 要点 a），其余表压缩为 dense embedding（要点 b），相比 RDL [9] 的全异构图建模会损失多跳关系信息；作者坦承这是为简化而做的取舍（§3 开头），并未给出与 RDL 的直接实验对比来量化这一损失。
- **LLM 作用被刻意边缘化**：论文反对直接用 LLM 处理关系表（成本论证 §1 + Figure 1），因此 LLM 仅作为 Predictor/Enhancer 出现，未探索 LLM + 结构化数据的更深融合（如 LLM 端到端表理解、table-grounded reasoning）；这是方法论选择而非技术突破。
