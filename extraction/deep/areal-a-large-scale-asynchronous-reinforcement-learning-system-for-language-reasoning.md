...
> 公式权威源 = extraction/formulas.json LaTeX，下文核心公式直接 `$$` 引用，不凭训练知识补全。
...
- **on-policy 约束与效率冲突。** PPO/GRPO 经典要求 latest-policy 数据（§3.1，标准 PPO 目标 Eq.2 用 $\pi_\mathrm{old}$ 作采样策略），现有系统 [27,11,45,44] 全同步；少数异步尝试 [30,24,49] 仍 batched generation、rollout 模型版本只能落后 1-2 步，generation 阶段低效未解决（§1, §2 "Asynchronous RL"）。
- **算法-系统耦合的硬骨头。** 异步后单条 trajectory 可能由多个 policy 版本分段产生（interruptible generation 导致），直接破坏标准 PPO 的 $\pi_\mathrm{old}$ 单策略假设（§4.2 "Inconsistent Policy Versions"；§5.2 Proposition 1 给出等价行为策略的构造性证明）。
...
1. **全异步解耦架构（generation vs training 完全分离）。** 4 个核心组件（§4.1, Figure 2 p.4，M3 要点）：Interruptible Rollout Worker（处理 generate 与 update_weights 两类请求）、Reward Service（CPU 线程跑 rule-based reward，如代码任务跑单元测试）、Trainer Worker（从 replay buffer 持续采样到 batch size 即做 PPO 更新，数据只用一次保证新鲜度）、Rollout Controller（桥接三者，跟踪 $N_r$ 与 policy version $i$）。M3 对 Figure 2 的解读强调"two decoupled GPU clusters ... coordinated via a Replay Buffer and Parameter Service"，prompts/trajectories 绿蓝流经 Rollout Coordinator，rewards 由 CPU Reward Service 回流——这正对应 §4.1 文字"asynchronous pipeline ensures continuous full utilization of both generation and training resources"。**LaTeX↔M3 双源校验**：M3 描述的"Parameter Service emits interrupt signals (red) and parameter save/load updates (purple) back to the rollout workers, closing the async loop"与 §4.1 的 update_weights 请求机制文字完全吻合，架构图无歧义。关键效果：**generation 和 training 在不同 GPU 集群上持续满载**，彻底消除同步系统里 context switching / weight resharding 在 critical path 上的开销（§2 末："completely eliminating resharding overhead from the critical training path"）。

2. **Interruptible Generation（可中断生成）。** 当 update_weights 到来，rollout worker 中断所有进行中的 generation，丢弃旧权重算出的 KV cache 并用新权重重算，然后继续 decode 未完成序列直到下一次中断或终止（§4.1，Figure 3 p.4 展示垂直 ready-time 线与蓝色 interrupt cross 的时序交错）。效果：**长 trajectory 的生成长度被隐藏在 critical path 之外**（§7.3："The generation of long responses can be fully hidden in the critical path, so extending generation length does not drastically affect the effective training throughput"）。消融（§7.5, Figure 6b p.10，M3 解读）：1.5B 231k vs 207k tokens/s（+12%）、7B 130k vs 111k tokens/s（+17%）生成吞吐提升。代价是引入"单条 trajectory 由多 policy 版本分段"的新算法挑战（§4.2），由下文 Decoupled PPO + Proposition 1 解决。

3. **Staleness-Aware Training（数据时效性控制，超参 η）。** §5.1 给出核心约束（formulas.json [3]，Eq.3，直接引用）：

$$
\lfloor (N_r-1) /B \rfloor \leq i + \eta.
$$

其中 $N_r$ 为已生成轨迹数、$B$ 为每步训练 batch size、$i$ 为当前 policy version。机制：Rollout controller **对新 generation 请求做速率限制**满足该式，并**优先从 buffer 取老 trajectory 组 batch**；$\eta=0$ 退化为同步 RL。要点（§5.1 末）：$\eta$ 太小会被极端长 trajectory 拖慢 generation 吞吐，故经验上**用较大 $\eta$ 换系统吞吐**——这反向倒逼算法层要能容忍更 stale 的数据。默认 $\eta=4$（code）/ $\eta=8$（math）（§7.1）。Figure 5c（p.9，M3 解读）以柱状图单调上升曲线量化了这一权衡：$\eta$ 从 $0\to\infty$，effective throughput $128.7\to 396.8$ k tokens/s，$\eta=4$ 已达 $356.6$（≈2.77× 于 $\eta=0$）。

4. **Decoupled PPO Objective（解耦 PPO，行为策略 vs 近端策略分离）。** §5.2 Eq.4–5（formulas.json [4]，直接引用）：

$$
J(\theta)= \mathbb{E}_{q\sim\mathcal{D}, a_t\sim\pibehav} \left[ \sum_{t=1}^H \min( \underset {{\text{Importance Ratio}}} { \boxed{ \frac{\pi_\theta}{\pibehav} } } \hat{A}_t,\quad \overbrace{ \frac{\piprox}{\pibehav} \mathrm{clip}( \underset{\text{Trust Region Center}}{ \boxed{ \frac{\pi_\theta}{\piprox} } } ,1-\epsilon,1+\epsilon )\hat{A}_t }^\text{Importance Ratio} ) \right]
$$

$$
= \mathbb{E}_{q\sim\mathcal{D}, a_t\sim\pibehav} \left[ \sum_{t=1}^H \frac{\piprox}{\pibehav} \min\left( u_t^\mathrm{prox}(\theta)\hat{A}_t, \mathrm{clip}\left( u_t^\mathrm{prox}(\theta),1-\epsilon,1+\epsilon \right)\hat{A}_t) \right),
$$

其中 $u_t^\mathrm{prox}(\theta)=\pi_\theta(a_t|s_t)/\pi_\mathrm{prox}(a_t|s_t)$ 是相对近端策略的 importance ratio。机制：PPO clip 中心从旧 $\pi_\mathrm{behav}$（低质量、滞后）挪到近端高质量 $\pi_\mathrm{prox}$，避免把最新 $\theta$ 拉向旧版本低质量策略（§5.2 "thus stabilizing training"）。对比标准 PPO Eq.2（formulas.json [2]）：
...
Eq.2 假设所有 action 由单一 $\pi_\mathrm{old}$ 采样；Eq.5 用 $\pi_\mathrm{behav}$（可含多版本）替代并引入 $\pi_\mathrm{prox}$ 作 trust region center，这是从同步到异步的关键一步。**Practical Remark**：Hilton et al. [10] 用参数 EMA 作 $\pi_\mathrm{prox}$ 对 LRM 不可承受，AREAL 简化为"用每步更新前的参数"作 $\pi_\mathrm{prox}$，在每个 training step 全局 batch 到达时 recompute token 概率实现。**Proposition 1**（§5.2，证明 §D）证明 interrupted 多版本 trajectory 等价于从单一构造的 $\pi_\mathrm{behav}$ 采样——其构造即 formulas.json [0]：
...
即按时间分段切换到当时实际生成该 token 段的那个 $\pi_{\theta+j}$——这是算法正确性的理论支柱。Figure 5a vs 5b（p.9，M3 解读）以 learning curve 形式给出最直接证据：5a naive PPO 在 $\eta=4$ 时 reward 曲线塌陷，5b 加 decoupled 后所有 $\eta\leq 8$ 曲线紧密聚集在 oracle($\eta=0$) 附近。**LaTeX↔M3 双源校验**：M3 对 Figure 5 的解读明确"panel (b) reproduces the same experiment with the decoupled PPO objective (eq. 5), where curves cluster tightly near the oracle (η=0)"，与 §5.2 文字及 Eq.5 的 trust-region 机制一致；M3 还指出 panel (c) throughput "rising monotonically from 128.7 (η=0) to 396.8 (η=∞)"，与 formulas.json [3] 的 staleness 约束给出的 $\eta$ 调节语义吻合。

5. **动态微批分配（Dynamic Microbatch Allocation / padding-free sequence packing）。** §6 + Algorithm 1（§B.3）：对变长序列按长度降序排序，贪心分配到能容纳的、序列数最少的微批，在固定显存约束 $C$、最少微批数 $k_\min$ 下均衡 token 分布。32,768 tokens/micro-batch budget（32 个标准 micro-batch 作对照）。效果（§7.5, Figure 6a p.10，M3 解读）：1B/7B/32B 平均 **+30% 吞吐**，M3 读数为 427.4 vs 404.4（1B）、454.7 vs 303.1（7B，差距最大 ~50%）、387.7 vs 283.0（32B）TFLOP/s。**LaTeX↔M3 双源校验说明**：M3 caption 在转写 7B/32B 的 normal 值时有错位（文字称 7B-normal=404.4、32B-normal=303.1），与 §7.5 正文"average of 30% improvements"及原图刻度读数（7B-normal=303.1、32B-normal=283.0）不符；此处以原文图刻度 + 正文为准校正。这是 M3 caption 自动解读的可靠性边界案例（见局限 #9）。
...
8. **端到端效果：2.77× 加速 + 性能不降反升。** §7.2 Table 1（见下表）。§7.3 strong-scaling（Figure 4 p.8，M3 解读）：2×3 网格（行=16k/32k ctx，列=1.5B/7B/32B），AReaL 蓝实线贴近黑色虚线理想线性 scaling，verl 橙虚线随 GPU 数增加逐步下坠；32B+32k 一格 verl 完全缺失（OOM）。最高 2.5× 吞吐 vs verl；线性 scaling 到 512 GPU。**LaTeX↔M3 双源校验**：M3 对 Figure 4 的解读明确"verl consistently encounters OOM with 32k context length and the 32B model so the data points are missing"，与 §7.3 正文及 Figure 4 caption 完全一致——这是 verl 作 baseline 在大模型长上下文场景的数据缺口。

**背景公式（formulas.json [1]，Eq.1，RL 基础目标，供参照）：**
...
关键读数（与 Figure 5a/5b 视觉互证）：$\eta=4$ 时 naive PPO AIME24 崩到 23.3（5a 曲线塌陷）、AMC23 崩到 58.5、MATH500 崩到 66.9，加 decoupled 后 AIME24 拉回 42.2（≈oracle 42.0）、AMC23 85.1、MATH500 89.5（下划线=±1 oracle）。证实 staleness control 与 decoupled objective 两者缺一不可；即使 decoupled，$\eta=\infty$ 仍劣于 oracle，说明 $\eta$ 必须有界——这与 Eq.3 的 staleness 约束语义一致。
...
随 $\eta$ 增大 throughput 单调上升（M3 确认 rising monotonically from 128.7 to 396.8）；$\eta=4$ 已达 ~2.77× 于 $\eta=0$。结合 Table 2：$\eta=4$ 是"吞吐已近 plateau + decoupled 后精度≈oracle"的甜点，印证 §7.1 默认 $\eta=4$ 的选择。
...
| 算法 | Decoupled PPO + staleness 控制（Eq.3,5） | 标准 PPO/GRPO（Eq.2） | off-policy RLHF | 标准 GRPO/PPO | 异步 PPO |
...
4. **η 选择仍经验化。** code η=4 / math η=8（§7.1），Table 2 + Figure 5a 显示 η=4 naive PPO 在 AIME24 崩到 23.3（decoupled 救回），说明 η 与算法耦合敏感；Eq.3 只给硬约束上界，无自动调参机制。
...
8. **算法适配边界。** Table 8 显示 RLOO 比 vanilla PPO 略更耐异步；作者明示 PPO 的 importance-sampling 项（Eq.5 中的 $\pi_\theta/\pi_\mathrm{behav}$）是异步 off-policy 的天然适配点，REINFORCE-like 与其他 off-policy 算法的异步耐受性是开放问题（§C.4 末）。
9. **M3 caption 与原图刻度的转写偏差。** Figure 6a 的 M3 caption 在 7B/32B 的 normal 值上与原图刻度存在错位（见 Table/Figure 6a 注），凡引用该图数字须以原文图轴为准——这是一个"图表 caption 自动解读"的可靠性边界案例。其余 Figure 2/4/5 的 M3 解读与原文文字、formulas.json LaTeX 三源一致，未见冲突。
