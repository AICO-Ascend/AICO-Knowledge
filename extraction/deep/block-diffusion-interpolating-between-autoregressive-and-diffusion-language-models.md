# Block Diffusion: Interpolating Between Autoregressive and Diffusion Language Models — 技术点深读（DEEP 2026-08-18，公式重跑）

> 独立文件，extract_phase1 重跑不丢。全要素深读 + M3 图文交叉验证 + formulas.json LaTeX 原文引用（权威源）。
...
> 公式权威源：extraction/formulas.json（下方 LaTeX 逐字引用，`$$` 包裹，未改写/未补全）。
...
AR 基线的似然分解（formulas.json [1]，Eq.1，作为插值的左端点）：

$$
\log p_\theta(\x) = \sum_{\ell=1}^L \log p_\theta(\xl \mid \x^{<\ell}),
$$

D3PM 反向过程（formulas.json [2]，Eq.2）：
...
D3PM NELBO（formulas.json [3]，Eq.3）：
...
- **机制**：把长度 L 序列分为 B 个长度 L′ 的块（B = L/L′）。似然按块分解（formulas.json [4]，Eq.4）：

$$
\log p_\theta(\x) &= \sum_{b = 1}^{B} \log p_\theta(\x^{b} \mid \x^{<b}),
$$

每个条件 $p_\theta(\x^b \mid \x^{<b})$ 由一个块内离散扩散过程建模（formulas.json [5]，Eq.5）：

$$
p_\theta(\x_s^b \mid \x_t^b, \x^{<b}) = \sum_{\x^b} q(\x_s^b \mid \x_t^b, \x^b)p_\theta(\x^b\mid \x^b_t,\x^{<b})
$$

对每块套 NELBO 得块扩散上界（formulas.json [6]，Eq.6）：
...
是合法 NELBO 上界。
- **关键插值性质**：L′=1 时退化为纯 AR NLL（Suppl B.4 证明，见创新点 7 公式 [17]）；L′=L 时退化为纯扩散。BD3-LM 在两端间连续插值，NELBO 随块增大变松（Suppl B.5：$\mathcal{L}_1 \leq \mathcal{L}_2 \leq \dots$，越细分越紧，对应 formulas.json [18]/[19]）。这正是 Figure 1（p.2）所绘「插值」位置——BD3-LM 处于 AR 与 Diffusion 之间，且 M3 解读明确其同时占得 KV caching + Arbitrary-length + Parallelizable + High quality 四项。

### 2. 块因果注意力 + KV cache 架构（§3.1, Eq.7）
- **机制**：单一 transformer $x_\theta$（block-causal mask）参数化全部 B 个去噪器。块 b 的 token attend 到块 1…b。关键签名（formulas.json [7]，Eq.7）：

$$
\x_\text{logits}^b, \mathbf{K}^b, \mathbf{V}^b \gets \x^b_\theta(\x^b_t, \mathbf{K}^{1:b-1}, \mathbf{V}^{1:b-1}) := \x^b_\theta(\x^b_t, \x^{<b}),
$$

去噪预测 $\x_\text{logits}^b$ + 同时吐出本块 $\mathbf{K}^b,\mathbf{V}^b$，并接收历史块 KV（等价于以 $\x^{<b}$ 为条件）。
- **效果**：与 AR 一致地支持 **KV cache**——采样新块时复用 $\mathbf{K}^{1:b-1}, \mathbf{V}^{1:b-1}$，无需重算。这是相对纯扩散（双向、无法 cache）的根本效率改进，对应 Figure 1（p.2）M3 解读中 Block Diffusion 行的 "KV caching" 勾选项。

### 3. 高效训练算法：双通 + 向量化单通（§3.2, Alg.1, Suppl B.6–B.7）
- **挑战**：去噪 $\x^b_t$ 需对噪声输入前向，而去噪下一块需对 *干净* $\x^b$ 前向——每块至少过网两次。
- **Alg.1 训练**：①对全序列干净前向 $(\emptyset, \mathbf{K}_{1:B}, \mathbf{V}_{1:B}) \leftarrow x_\theta(x)$ 预算 KV；②对每块 $\x^b_{\text{logit}} \leftarrow x^b_\theta(\x^b_t, \mathbf{K}_{1:b-1}, \mathbf{V}_{1:b-1})$ 算去噪 logits。每 token 过网两次。
- **向量化单通**（§3.2, Suppl B.6）：把 $\x_{\text{noisy}} \oplus \x$（长 2L）拼接送一次前向，配自定义注意力掩码（formulas.json [0]，Suppl B.6）：

$$
\M_{\text{full}} = \begin{bmatrix} \M_{BD} & \M_{OBC} \\ \mathbf{0} & \M_{BC} \end{bmatrix}
$$

其中 $\M_{BD}$=块对角（噪声块内自注意）、$\M_{OBC}$=偏移块因果（噪声块 attend 到更早的干净块）、$\M_{BC}$=块因果（干净块更新表示）。避免 B 次 loop。
- **Figure 3（p.21）图文交叉验证**：M3 解读该图为 L=6、L′=2 的 6×6 注意力掩码实例，三个结构区域清晰可分——左上 3×3 块对角 $\M_{BD}$（橙色，$\x^1_t,\x^2_t,\x^3_t$ 仅 attend 本 2-token 块内做去噪）；右上 3×3 偏移块因果 $\M_{OBC}$（蓝色，噪声 token cross-attend 更早的已定稿条件块）；下方 3×6 块因果 $\M_{BC}$（黄色，干净 token $\x^1,\x^2,\x^3$ 按标准块因果更新表示）。M3 takeaway 与 §B.6 公式 [0] 定义一致：掩码块级结构产生极端稀疏性，是后续 FlexAttention 单核融合的前提。
- **Figure 4（p.22）代码交叉验证**：M3 解读该图为 `block_diff_mask` 的 PyTorch 实现，操作 2n 长度的 q_idx/kv_idx（拼接 $\x_t$ 与 $\x_0$）。数据流：①用 `x0_flag_q/kv`（index≥n 判 $\x_0$）标记 token 归属；②用 `block_size` 把位置映射到块（$\x_0$ 用 index−n 偏移）；③三个布尔子掩码 OR 合成——$\M_{BD}$（同块且同侧）、$\M_{OBC}$（$\x_t$ 块号 > $\x_0$ 块号且 query 在 $\x_t$ 侧/key 在 $\x_0$ 侧）、$\M_{BC}$（query/key 均在 $\x_0$ 侧且块号 ≥）。与 §B.6 的数学定义逐条对应，验证了实现忠实于推导。
...
- **Alg.2 采样**：逐块采样 $\x^b \leftarrow \text{SAMPLE}(\x^b_\theta, \mathbf{K}^{1:b-1}, \mathbf{V}^{1:b-1})$，每采完一块做干净前向 $(\emptyset, \mathbf{K}^b, \mathbf{V}^b) \leftarrow x^b_\theta(\x^b)$ 累积 KV。块内用任意扩散采样器（cross-attention 注入历史 KV）。
...
- **观察**：L′=1 时 BD3-LM 在期望上与 AR NLL 等价（Suppl B.4 严格证明，见创新点 7 公式 [17]），但在 LM1B/16B tokens 上竟有 **~2 点 PPL 差距**（Table 1：BD3-LM L′=1 ≤25.56 vs AR 22.88）。
...
- **batch NELBO 估计器**（formulas.json [8]，Eq.9，batch size K）：
...
- **梯度方差估计器**（formulas.json [9] 为方差估计骨架，[10] 为 M-batch 经验方差，Eq.10）：
...
- **masked BD3-LM** 采用 Sahoo et al. 2024a (MDLM) 框架：per-token 前向 $q(\x^\ell_t|\x^\ell)=\text{Cat}(\x^\ell_t; \alpha_t \x^\ell + (1-\alpha_t)m)$，$\alpha_t$ 单调降 $\alpha_0=1,\alpha_1=0$。扩散矩阵 $\mathbf{Q}_t$、$\mathbf{Q}_{t|s}$（formulas.json [10]、[11]，Eq.12–13）：
...
前向/反向过程（formulas.json [12]、[13]，Eq.14–15）：
...
- **扩散损失项**（formulas.json [14]，Eq.16/17 推导中间态）：
...
- **简化目标**（formulas.json [16]，Eq.8/19，是加权交叉熵之和；$T\to\infty$ 时成立）：
...
- **重建损失归零**（formulas.json [15]，Eq.18，$\x^b_{t(1)}\to\x^b$ 使 $\mathcal{L}_{\text{recons}}=0$；先验损失 $\mathcal{L}_{\text{prior}}=0$ 因 $\alpha_{t=1}=0$）：

$$
\mathcal{L}_{\text{recons}} &= - \mathbb{E}_q \log \p (\x^b | \x_{t(1)}^b, \x^{<b}) \nonumber \\ &= - \log \p (\x^b | \x_{t(1)}^b = \x^b, \x^{<b}) \nonumber \\ &= 0
$$

- **NELBO 对调度不变**（Suppl B.3 末），但 Monte Carlo 估计器方差依赖调度——正是创新点 5/6 的抓手。carry-over 性质亦是创新点 4 中 NFE≤L 上界的依据。
- **L′=1 退化到 AR 的严格证明**（formulas.json [17]，Suppl B.4，Eq.20–21）：把 $\alpha'_t=-1,\alpha_t=1-t$ 代入，利用 $q(\x^b_t=m|\x^b)=t$ 与 carry-over 性质，最终得：
...
即单 token 生成（L′=1）恢复 AR NLL。这是「期望等价但方差不同」论证的形式化根基。
- **NELBO 紧性**（formulas.json [18]/[19]，Suppl B.5，Eq.22–23）：$\mathcal{L}_1 \leq \mathcal{L}_2 \leq \dots \leq \mathcal{L}_L$，由 Jensen 不等式逐级放缩得到（[18] 为 $\mathcal{L}_1$，[19] 为 $\mathcal{L}_2$）：
...
- **vs D3PM (Austin 2021)**（§7）：BD3-LM 建立在 D3PM 上并应用到每个 AR 条件块。三点改进——(1) 突破 D3PM 定长（对应 Figure 1 p.2 M3 解读中 Diffusion 行 "Fixed-length" 缺陷，BD3-LM 行改为 "Arbitrary-length"）；(2) 识别梯度方差为 PPL gap 贡献者并提出方差最小化调度（Figure 2 p.6 曲线可视化，公式 [8]/[9]/[10]）；(3) PPL 优于 D3PM（82.34 → 28.23）。适用 D3PM 各连续时间扩展（Campbell/Sun 2022）。
- **vs MDLM (Sahoo 2024a)**（§7）：BD3-LM 复用 MDLM 的简化 NELBO/SUBS 参数化（公式 [14]–[16]）。两点推进——(1) MDLM 指出 NELBO 对调度不变，本文揭示调度显著影响**梯度方差**；(2) PPL 超 MDLM（LM1B 31.78 → 28.23，OWT 22.98 → 20.73）。注意 PPL 提升部分来自优化调度而非仅块扩散，可反哺 MDLM/D3PM。生成质量上，Figure 6（p.26）MDLM 单样本 Gen.PPL=69.26 主题漂移严重，Figure 7（p.27）BD3-LM L′=16 同设置 Gen.PPL=24.3 且 M3 定性观察连贯性显著优于 MDLM——直接定性证据支持 §6.2 "BD3-LM samples have higher coherence than MDLM"。
- **vs SSD-LM (Han 2022) / 高斯嵌入扩散**（§7, §6.2）：SSD-LM 在连续 embedding 上做高斯扩散的块扩散，不支持 tractable likelihood；BD3-LM 离散扩散支持似然估计。生成效率——BD3-LM NFE 上界 L（carry-over，公式 [15]），SSD-LM 每块 T 步共 BT 步（T=1K,L′=25 → 40K NFE）。Table 7：BD3-LM L′=4 用 1K NFE 达 25.7 Gen.PPL，SSD-LM 需 40K NFE 仅达 37.2；NFE 可比时（T=25）SSD-LM 崩到 281.3。连续 embedding 高斯扩散整体 PPL 更差（引 Graves 2023, Gulrajani 2024）。
...
- → **[[dspark-confidence-scheduled-speculative-decoding-with-semi-autoregressive-generation]]**（DSpark, #8）：同属半 AR 家族，引入 confidence-scheduled 调度，与 BD3-LM 的 data-driven clipped schedule 在「调度驱动半 AR」思想上同源。BD3-LM 的梯度方差估计器（公式 [8]/[9]/[10]）与网格搜索 β,ω（§5.3）是 DSpark confidence scheduling 的概念前驱。
- 与 **[[medusa-simple-llm-inference-acceleration-framework-with-multiple-decoding-heads]]**(#2) + **[[eagle-speculative-sampling-requires-rethinking-feature-uncertainty]]**(#4)：同处推测解码谱系的另两条分支（多头自推测 / 特征级单头 draft）。Block Diffusion 把「并行性」放在块内扩散而非 draft-then-verify 树，KV cache 复用路径与 EAGLE 系列不同（EAGLE 复用 target 末层特征，BD3-LM 复用历史块 KV，公式 [7]）。
...
- **NELBO 随块增大变松**：Suppl B.5 证明（公式 [18]/[19]）$\mathcal{L}_1 \leq \mathcal{L}_K$，块越大似然上界越松——L′=L（纯扩散）PPL 最差，L′=1（纯 AR）最紧。存在「块越大并行度越高但似然越松」的根本权衡。
