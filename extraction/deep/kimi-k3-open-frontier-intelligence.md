...
开源 LLM 在 **test-time scaling（第二轴）** 上进展迅速（RL/长推理/agent），但 **pre-training foundation（第一轴）** 长期停滞在 1T-class 参数区间，导致"开源之间趋同、与最强闭源（Claude Fable 5 / GPT-5.6 Sol）差距拉大"。同时，把 scaling 两轴同时推进到 3T-class + 1M 上下文 + 长程 agentic RL，撞上三重系统瓶颈：(i) 长序列注意力的 KV cache 与算力二次膨胀；(ii) 2.8T MoE 在 expert-parallel 训练下的负载不均、内存爆炸、视觉编码器抖动；(iii) million-token agentic rollout 的尾延迟、KV cache 留存、沙箱状态持久化。

**Kimi K3 的回答**：在三个维度同时 scale 信息流——序列长度（Hybrid KDA+MLA）、网络深度（Attention Residuals）、宽度（Stable LatentMoE，16/896 路由专家）——配合原生多模态训练与 multi-effort agentic RL，叠加全栈基础设施（FlashKDA kernel / KDA Context Parallelism、MoonEP 完美均衡 EP、co-located 长上下文 RL + AgentENV microVM 沙箱、KDA-aware 前缀缓存）。结果：**2.8T 总参 / 104B 激活 / 1M 上下文**，相对 K2 scaling efficiency 提升 **~2.5×**（Fig.7，p.11：在 log-FLOPs vs validation loss 的 scaling-law 曲线上 K3 整条曲线低于 K2，横向 2.5× 箭头标注，即达到同等 loss 所需 FLOPs 缩 2.5×）；开源 frontier 模型，权重全放，整体仅次于 Claude Fable 5 与 GPT-5.6 Sol，但显著领先其它开源与闭源对照（Fig.1，p.1：12 项基准横向条形图，K3 蓝条在 SWE-Marathon 上 42.0 vs 次优 35.0 领先最显著）。
...
### Table 1（§3.2, p.11）：Kimi K2 vs Kimi K3 架构对比
| 项 | Kimi K2 | Kimi K3 | Δ |
|---|---|---|---|
...
### Table 2（§6.1.4, p.27，公开基准，max effort；HLE/MMMU-Pro/CharXiv/Math-Vision/ZeroBench 为 w/o tool / w/ tool）
...
注：Fable 5 全部带 fallback、GPT-5.6 Sol 全部带 cyberguards（Fig.1 注、Table 2 注）。†ALE leaderboard 上 Fable 5 跑 xhigh effort 且 40% 任务标 downgraded。

### Table 3（§6.2.1, p.29）：内部基准（max effort，GPT-5.5 xhigh；harness 见 Harness 列）
| Benchmark | Harness | Kimi K3 | Fable 5 | GPT-5.6 Sol | Opus 4.8 | GPT-5.5 | GLM-5.2 |
|---|---|---|---|---|---|---|---|
| Kimi Code Bench 2.0 | Claude Code | 73.7 | 76.9ᵃ | – | 71.7 | – | 64.2 |
| Kimi Code Bench 2.0 | Kimi Code | 72.9 | – | – | 66.0 | – | – |
| Kimi Code Bench 2.0 | Codex | – | – | 64.8ᵇ | – | 69.0ᶜ | – |
...
| Faithfulness (1−halluc)ᵍ | N/A | 85.5 | – | 84.8 | 83.6 | 86.5 | 74.8 |
| Chat All-in-One Bench | Kimi Work | 85.2 | 88.0 | 79.0 | 83.8 | 71.8 | – |

ᵃ13 fallbacks + 1/80 refusal；ᵇ10/80 refusals；ᶜ3/80 refusals；ᵈ含 2 Fable 5 refused；ᵉ含 14 Fable 5 refused；ᶠ6/95 refusals；ᵍmetric = 1−hallucination rate，越高越好。

### Table 4（§6.2.1, p.29）：Kimi Webdev Bench vs Claude Opus 4.8（盲评专家）
...
### Table 5（§6.3, p.31）：第三方评测（as of July 23, 2026）
| Benchmark | Kimi K3 | Fable 5 | GPT-5.6 Sol | Opus 4.8 | GPT-5.5 | GLM-5.2 |
|---|---|---|---|---|---|---|
...
| Text Arena (Elo) | 1,486 (#8/200) | 1,507 | 1,485ᵃ | 1,484ᵇ | 1,482ᵇ | 1,469 |
| Agent Arena | 9.1 (#4/37) | 12.7 | 10.1 | 9.8 | 8.8 | 6.5 |

### 关键效率数字（§6.4, Figure 13 p.32）
...
- M3 对 Fig.13（p.32）的解读：2×2 散点图，每面板 quality metric vs cost/task(USD)，左上角=成本效率前沿更优。K3（红星）在四套件上均落于 Pareto 前沿——KCB 2.0 ~$2 vs Fable 5 高成本；BrowseComp 含 low/medium/high/max effort 变体与 1M/3M/10M token 预算点；GDPval-AA v2/AA-Briefcase K3 在 Elo 接近顶级的同时成本显著低。

### 案例研究（§7）核心数字
- **GPU kernel 优化（§7, Fig.14 p.33）**：AttnRes 283.6ms→114.4ms，DSA −55.1%，KDA −73.6%，MLA 超 peak TFLOPS 一半；match Fable 5(with fallback)，超 Opus 4.8/GPT-5.6 Sol/GPT-5.5。M3 对 Fig.14 解读：AttnRes 上 speedup vs FLA Triton baseline(%) 随 active hours 增长，K3（红）~13h 达 ~+59.7%、最早最陡爬升；Fable 5 ~+57.1%（~15h）；GPT-5.5 +30.8%（~20h）；GPT-5.6 Sol +17.3%（~18h）。
- **MiniTriton 编译器（§7, Fig.15 p.34）**：matmul 近 cuBLAS 约 90% machine roof；DSL-level KDA prefill kernel 超 Triton reference；GPT 训练 loss 紧贴 PyTorch reference，全模型梯度 vs torch autograd 差 ≤ fp32 rounding 10⁻⁴（fp64 参考）；DDP 2×L20 vs 单卡 final 2.4870 vs 2.4876，max|diff| 0.0033/mean 0.0003。M3 对 Fig.15 解读：(a)(b) L20 sm_89 上 CUDA-core fp32 / tensor-core tf32+bf16 roofline，MiniTriton 在 matmul/softmax/flash_attn/kda/gpt50m_step 上贴或近 roofline；(c) character-level GPT 100 步 loss 曲线紧贴 torch eager；(d) 两卡 NCCL DDP vs 单卡同 init/batch/LR 曲线重合。
- **芯片设计（§7）**：48h 自主跑，nano 模型推理芯片原型（同 hybrid KDA+NoPE-MLA、Block AttnRes block size 2、sigmoid MoE routing、1 shared expert、group-wise INT4 w128），4mm² 内 100MHz 闭时序、RTL 仿真 decode >8,700 tokens/s、1.46M cells、0.277 MiB SRAM、INT4 MAC 融合反量化。
...
- **vs [[kimi-k2-open-agentic-intelligence]]（直接前代）**：K2 是 1.04T/32.6B 激活/128K 上下文/全 MLA/SwiGLU/384 路由专家 8 激活/1 shared。K3 在所有维度加码（2.78T/104.2B/1M/Hybrid KDA-MLA/SiTU-GLU/896 路由 16 激活/2 shared），加 AttnRes 跨层、Stable LatentMoE 稳定化、QB 路由均衡、MoonEP 完美均衡、MoonViT-V2 从头训、microVM 沙箱、KDA-aware 前缀缓存。scaling efficiency ~2.5×（Fig.7）。
- **vs [[kimi-k2-5-visual-agentic-intelligence]]（K2.5）**：K2.5 引入视觉 agentic + Agent Swarm；K3 继承 K2.5 视觉通路设计但**视觉编码器从头训**（K2.5 用 SigLIP-init 的 MoonViT-3D，Fig.6 显示其梯度 spike），骨干换 hybrid attention，扩展到 3T。test-time scaling 由 K2.5 的并行 agent swarm 推进到 K3 的多力度 RL + MOPD 统一。K3 的 multimodal encoder 优化延续 K2.5 的 DEP 思路并进一步分解 ViT 塞进 PP bubbles。
- **vs [[deepseek-v4-towards-highly-efficient-million-token-context-intelligence]] / [[deepseek-v3-technical-report]]（DeepSeek frontier 线）**：DeepSeek 系是 MLA 的起源（V2 引入，V3/V4 采用并扩到 MoE）。K3 保留 MLA 作为周期性全局注意力层，但主体换成 KDA 线性注意力 + AttnRes 跨层混合——是 DeepSeek 路径之外的一种"长上下文靠线性注意力 + 少量全局层"的替代架构选择。K3 的 LatentMoE（compact latent 空间路由）+ auxiliary-loss-free QB 也与 DeepSeek 的 MoE 路由思路形成对照（DeepSeek V3 用 auxiliary-loss-free 变体，K3 进一步推到 QB 闭式解）。K3 强调"开源 3T-class 首例"，DeepSeek-V4 强调"高效 million-token 上下文智能"，二者在长上下文 + MoE 上方向同向但架构解不同。
