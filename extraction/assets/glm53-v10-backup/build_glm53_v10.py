...
def sec(label, inner, pad='46px 72px', bg='#fff', extra=''):
    return ('<div class="slide-fit" data-idx="" data-active=""><div class="slide-canvas">'
            ...
            f'background:{bg}; font-family:\'Noto Sans SC\',sans-serif; overflow:hidden; position:relative;{extra}">'
...
def chapter(code, title, desc, kws):
    ...
    return sec(f'章扉{code}', (
        ...
        extra=' display:flex; flex-direction:column; justify-content:center;')
...
# ═══════════════════════════════════════════════════════════
add('目录', sec('目录', (
    eyebrow('CONTENTS') + h2('5 章 35 页 · 一图速览', 42)
    + '<div style="display:flex; gap:18px; margin-top:18px;">'
    + pillar('CH 1', BLUE, '背景与结论', 'GLM-5.2 → 5.3-flash 的 8 维结构差异 · 45 层混合注意力排布 · 线性注意力科普（3 页）')
    ...
    + pillar('CH 4', '#8a6a2b', '性能优化阶梯', '958.3 → 39.7ms · 图模式 / fla_npu / 融合算子 / gather 134× / MTP（7 页）')
    + pillar('CH 5', '#6a4a8c', '工程化交付', 'TP8 拉起脚本 · 评测数据 · 5 处结构变化 × 问题 × 手段总结（3 页）')
    + pillar('收尾', GREY, '深读 · 术语 · 文献', 'KB 同源论文原图深读 · 术语速查 · arXiv 参考文献（5 页）')
    + '</div>'
    + foot('★ 页数口径：卡片标注为各章内容页（3+7+3+7+3+5 = 28 页）+ 封面 / 目录 / TLDR / 4 张章扉 = 全 35 页。'
           '阅读动线：技术小白建议顺序读（每章开头有科普）；工程师可直接跳 CH 3 / CH 4 看量化盲区与性能数字。')),
    extra=' display:flex; flex-direction:column; justify-content:center;'))
...
add('TLDR', sec('TLDR', (
    ...
    + ''.join(
        ...
        for k, v, d, c in [
            ...
            ('吞吐', '22.3 tok/s', '单并发口径；基线 1.04 → 22.3 = 吞吐 21×（TPOT 口径 24.1×，官方取「24 倍」）· MTP verify ~2 token/步', BLUE),
...
add('ch1-对比表', sec('ch1-对比表', (
    ...
    + dtable(['维度', 'GLM-5.2', 'glm5.3-flash'],
        [['注意力', '全程 MLA+DSA', '混合：KDA(34 层) + DSA(11 层)'],
         ...
         ['模态', '纯文本', 'VLM 图文（--backend=vlm）'],
         ['复用', '—', 'MoE / MLP / MLA / W8A8 复用自 DSV3.2']],
        hl=2, fs=13, col_w=[1, 1.6, 1.8])
    + foot('★ 背景：开源 vLLM 在 GLM-5.3 上精度未对齐、0day 未发布 — 本次 xllm/uniinfer 适配是线上可用的唯一路径（官方交付实录 · jx.huawei.com）。'))))
...
out = S · q           # ④ 查询：取状态作为输出''',
    ...
    + kbstrip(U['kb-k3-3'],
        'Kimi K3 Fig.3 给出 KDA 家族的两个关键设计证据：<b>图(a)</b> 对数衰减参数化对比 — '
...
add('ch2-kda', sec('ch2-kda', (
    ...
    + lead('LayerCache 为 KDA 新增 conv/ssm 两槽；SSM 64×128×128 fp32 = 4MiB/序列定长。'
    ...
    + figcap(U['fig03-kda-deltarule'],
        ...
        '口径：glm5.3-flash uniinfer 实现 · conv [slots, 3, 3072] fp32 (TP8) · ssm [slots, 8, 128, 128] fp32（TP8 每 rank 8 heads × 8 = 全量 64 heads）',
        img_pct=50)
    + codebox(
...
class LayerCache:
    ...
    ssm:   torch.Tensor | None = None  # KDA 线性层 [slots, 8, 128, 128] fp32 (TP8: 8×8=64 heads = 4MiB/序列)
...
add('ch2-kda-state', sec('ch2-kda-state', (
    ...
    + kbstrip(U['kb-gdn-t4'],
        'GDN（KDA 前身）Tab.4 是「<b>定长状态不丢精度</b>」的直接证据：6 项检索/QA 任务上，'
        'Gated DeltaNet 平均 30.6 居循环模型之首（Mamba2 29.8 · DeltaNet 26.2）；'
    ...
    + dtable(['', '标准 KV cache', 'KDA conv/ssm 状态'],
        [['写入语义', '追加 (append)', '原地递推 (read-modify-write)'],
         ...
         ['prefix cache', '天然支持（公共前缀跨请求复用）', '<b>难兼容</b>：conv/ssm 每 token 都变化（官方实录）'],
...
add('ch2-mhc', sec('ch2-mhc', (
    ...
    + kbstrip(U['kb-hc1'],
        'mHC 论文 Fig.1 是本页结构的直接出处：<b>(a)</b> 标准残差 — xₗ 经层后与自身相加；'
...
add('ch2-kpool', sec('ch2-kpool', (
    ...
    + lead('打分对象从「每个 token 的 index key」变成「每 4 token 一组的 pool_key」，选择空间 ÷4。'
           ...
           '<b>维数 128 → 257（×2+1），原 index 管理不可用、需整体重构</b>（官方交付实录）。')
...
add('ch3-pipeline', sec('ch3-pipeline', (
    ...
    + '<div style="font-size:15px; color:#333; line-height:1.7;">'
      ...
      '<b>③ 线性层 int8</b>（MLP 权重 + 激活双 8bit，gate 分支除外）→ <b>④ 导出</b>（w8a8 真实权重）。'
    ...
    + '</div>'
    + '<div style="flex:1;">'
    + laybox('量化 = 把高清照片压成 JPEG 省空间。直觉例子：x=[100, 0.5, −0.2, 0.3] 直接量化 ≈ [127, 1, 0, 0] — '
             '一个 outlier 吃掉全部刻度预算，其余维度被压没。QuaRot 的「旋转」先转个角度把能量摊平再压，压完不留明显瑕疵。'
             'attention 部分太敏感，干脆不压。')
    + '<div style="font-size:13.5px; color:#444; line-height:1.7; margin-top:10px; padding:10px 14px; background:#eef7ee; border-left:3px solid ' + GREEN + '; border-radius:6px;">'
    '<b>量化收益（官方交付实录）</b>：bf16 → w8a8 推理性能提升 <b>~50%</b>（权重访存减半）· 精度无损 '
    '— MMMU_PRO 78% / AIME2026 90% 全程无回退。注意：非线性操作（sigmoid/softmax/layernorm）之前必须先消除旋转再计算。</div>'
    + '</div></div>')))
...
add('ch3-rot', sec('ch3-rot', (
    ...
    + lead('σ(R·h) ≠ R·σ(h) — glm5.3-flash 新增的数据相关门控从旋转残差流取输入、输出喂 sigmoid/softmax，'
    ...
    + laybox('「旋转」像把整个城市地图转了 45° — 所有路牌（权重）都得跟着转。'
             ...
             '车照老路牌开，就开到错的地方去了。')
    + foot('★ 检查清单（官方交付实录）：任何「输入维 = hidden 的投影」一律右旋 W·R；'
           '非线性操作（sigmoid / softmax / layernorm / silu）之前必须先消除旋转再计算 — 权重左旋还是右旋逐位置确认。'))))
...
# f_a / f_b / g_a_proj 输入维 = hidden → 必须右旋；g_b 为二级（输入维=head_dim）→ 一级补旋后不旋
forget_gate.f_a_proj.weight.data = forget_gate.f_a_proj.weight.data @ R
forget_gate.f_b_proj.weight.data = forget_gate.f_b_proj.weight.data @ R
forget_gate.g_a_proj.weight.data = forget_gate.g_a_proj.weight.data @ R
# g_b_proj：二级投影，保持原权重不旋
...
add('ch4-graph', sec('ch4-graph', (
    ...
    + figcap(U['fig12-host-compute'],
        '左：eager 模式时间分布 — <b>NPU 真实工作仅 38%，62% 在等 host</b>'
        '（每次 .item() 同步 + 逐算子 launch + Python dispatch）；'
        '右：aclgraph 后 — host 占比 ≈0%，<b>compute 97%</b>（整模型录成静态图，一次 graph.replay()）。<br><br>'
...
add('ch4-kpool', sec('ch4-kpool', (
    ...
    + '<div style="display:flex; gap:14px; margin-top:12px;">'
    + pillar('中间张量', RED, '33.5MB → 0.25MB', '134× 压缩 — fancy index 的布尔掩码物化是显存与搬运双料杀手。')
    + pillar('调用次数', BLUE, '3354 → 33', 'AI_CPU 行级取数 → AI_VECTOR_CORE 查表，每层 Index 调用 ÷100。')
    + pillar('单层耗时', GREEN, '2.5 → 0.05ms', 'B=1, T=32768, D=128, rate=4 实测（官方分支微基准注释逐字命中）。')
    + '</div>')))
...
add('ch4-mtp', sec('ch4-mtp', (
    ...
    + lead('per-(layer, slot) 暂存原始 qkv/gate/β → 跨层批量前进 + lead-trim。'
           ...
           '<b style="color:' + RED + ';">前置条件：MTP 需先适配权重再开启，否则接受率=0、TPOT 劣化（官方交付实录）。</b>')
    + '<div style="display:flex; gap:22px; align-items:flex-start;">'
    + '<div style="flex:0 0 47%;">'
    + f'<img src="{U["fig14-mtp"]}" style="width:100%; border-radius:8px; border:1px solid {LIGHT};" />'
    + kbstrip(U['kb-dsv3-3'],
        'DeepSeek-V3 Fig.3 是 MTP 的原始架构：主模型以 t₁–t₄ 预测 t₂–t₅，MTP 模块 1/2 依次加深'
        '（预测 t₃–t₆ / t₄–t₇），<b>保留完整因果链</b>；每模块 = 共享 Embedding + RMSNorm 拼接 + '
...
add('ch5-summary', sec('ch5-summary', (
    ...
    + '<div style="display:flex; gap:14px; margin-top:14px;">'
    + pillar('交付 ①', RED, '代码合入官方分支', 'xLLM-AI/xllm @ preview/glm-5.3-flash（HEAD a6ae154）— 0day 适配全部落地。')
    + pillar('交付 ②', BLUE, '镜像 + 拉起脚本', 'quay.io/jd_xllm/xllm-ai 镜像 · 单机 8 卡 TP8 拉起脚本（含图模式 / chunked prefill）。')
    + pillar('交付 ③', GREEN, '技术分析帖', 'jx.huawei.com 内部帖子：状态管理 / 旋转量化 / KDA 与 kPool 性能瓶颈全记录。')
    + '</div>'
...
# ── zoom 填充后处理 (v10: fill2 img uid 正则修复后按真实内容底重调; 目录/章扉改垂直居中) ──
ZOOM = {
