...
> [!tip] 技术解读（多模态）
> 【图文联合解读】图(b)为IFBench/Qwen3 8B上的学习曲线对比：蓝线(GEPA)在前几百次rollout即陡升至近满分并保持高平台；绿线(MIPROv2)上升模式类似但终值略低；橙线(GRPO)增长极缓，24k rollouts后仍处低位徘徊。测试集星标(左上蓝星≈满分，右下橙星≈零分)直观显示泛化鸿沟。
...
> [!tip] 表格解读（多模态）
> 【图文联合解读】图仅含caption文字、未呈现表格数据，解读依据原文：

1) **结构**：以Qwen3 8B为底座，对比GEPA、GEPA+Merge与GRPO等优化器在IFBench、AIME等多基准上的得分与所用rollout数；
2) **关键结论**：caption量化举例——IFBench上GEPA仅用678次rollout即达38.61%，反超GRPO以24,000次rollout取得的35.88%；除AIME外，GEPA/GEPA+Merge均以远少rollout在所有基准上胜出；
3) **论文作用**：作为§4 Evaluation总览表，为"反思式提示进化可优于RL"这一核心论点提供跨基准量化证据，与Fig.1（学习曲线）形成"表—图"互补论证。
...
> [!tip] 表格解读（多模态）
> 【图文联合解读】表2以GPT-4.1 Mini为评测模型，对比7种优化器在HotpotQA/IFBench/Hover/PUPA/AIME-2025/LiveBench-Math六个基准上的聚合分：Baseline 53.03，Trace 56.30(+3.27)、MIPROv2 58.67(+5.64)、TextGrad 59.14(+6.11)，**GEPA以65.22(+12.19)居首，GEPA+Merge达66.36(+13.33)**。原文论证两点：(a) GEPA在闭源模型上开箱即用，全面超越MIPROv2等SOTA提示优化器；(b) **用Qwen3-8B弱模型优化的GEPA-Qwen-Opt在GPT-4.1 Mini上仍获+9.00，反超所有直接为GPT优化的基线**，证实其提示具强跨模型泛化性。该表作为主结果证据，支撑"提示优化可匹敌甚至超越RL"这一全文核心论点。
...
**核心数据：** 在Qwen3 8B上对比四种候选选择策略在HotpotQA/IFBench/Hover/PUPA四任务上的聚合得分——Baseline 48.84、SelectBestCandidate 54.89（+6.05）、BeamSearch 53.95（+5.11）、GEPA 61.28（+12.44）。GEPA在每个单项任务上均取得最高分。

**关键结论：** 论文借此消融验证，固定进化框架时，GEPA的Pareto式候选采样相比TextGrad式贪心（SelectBestCandidate）和APO式束搜索（BeamSearch）可分别多带来6.39和7.33个百分点的增益，且避免了局部最优。

**在论文链路中的作用：** 该表是GEPA核心组件——候选选择策略的消融实验，支撑Figure 3所述"Pareto-based candidate sampling"设计，证明多样性保留机制是GEPA超越既有prompt优化方法的关键所在。
