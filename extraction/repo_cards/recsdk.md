# 代码仓卡片 · recsdk

> 骨架由 repo_card.py 机械生成 (全部事实来自 inventory/docs_index);
> 「分析层」小节由 LLM 深读填写, 每个事实须带仓库内出处 (文件路径)。

## 0. 元信息

| 项 | 值 |
|---|---|
| 仓库 | https://gitcode.com/Ascend/RecSDK.git |
| 分支 / HEAD | `develop` @ `346b3d1e47fb` (2026-09-01) |
| 最新 tag | `v26.1.0` |
| 版本候选 |
  - git_tag: `v26.1.0`
| 文件数 / md 文档 / 图片 | 2171 / 266 / 107 |
| 语言分布 | {"Python": 519, "C/C++ hdr": 444, "C++": 348, "Markdown": 267, "Shell": 190, "YAML": 1} |
| 备注 | 华为昇腾-MindX 推荐SDK |

## 1. 定位 (LLM)

## 2. 架构与模块 (LLM, 基于下表 + 源码走读)

| 顶层路径 | 文件数 |
|---|---|
| `CMakeLists.txt` | 文件 |
| `LICENSE` | 文件 |
| `README.md` | 文件 |
| `README_en.md` | 文件 |
| `SECURITYNOTE.md` | 文件 |
| `Third_Party_Open_Source_Software_Notice` | 文件 |
| `build.sh` | 文件 |
| `build` | 文件 |
| `config.ini` | 文件 |
| `contributing.md` | 文件 |
| `cust_op/` | 935 |
| `dependency.xml` | 文件 |
| `docker/` | 11 |
| `docs/` | 290 |
| `pre-commit` | 文件 |
| `setup.py` | 文件 |
| `setup_common.py` | 文件 |
| `setup_tf1.py` | 文件 |

## 3. 功能逻辑 · 特性地图 (机械层: 0 篇特性文档聚类)

| 分组 | 数量 | 特性 |
|---|---|---|


## 4. 关键特性深读 (LLM 选 3-6 篇, 图文联合)

## 5. 版本与演进

  - Version Mapping
  - Product Version Information
  - Related Product Version Mapping
  - Virus Scan Results
  - Version Compatibility
  - Usage Precautions
  - Change Description
  - New Features
  - Interface Changes
  - Key Feature Changes
  - Resolved Issues
  - Known Issues

## 6. 文档地图 (机械层)

| 类型 | 数量 |
|---|---|
| readme | 114 |
| doc | 71 |
| api | 56 |
| overview | 14 |
| guide | 8 |
| faq | 6 |
| changelog | 2 |

## 7. 证据与出处

- 文档收割: extraction/repo_docs/recsdk/ (271 篇)
- 清单: extraction/repo_inventory.json · extraction/repo_docs_index.json
