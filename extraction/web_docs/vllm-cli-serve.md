...
:   Read CLI options from a config file. Must be a YAML with the following options: [configuration/serve\_args](https://docs.vllm.ai/en/latest/configuration/serve_args/)
...
:   If provided, the server will require one of these keys to be presented in the header.
    ...
    Warning: this only authenticates endpoints under the `/v1`, `/v2`, and `/inference` path prefixes. Other endpoints on the same server, including `/invocations` (which exposes the same inference capabilities as `/v1`), remain unauthenticated. Do not rely on `--api-key` alone to secure vLLM; see [usage/security](https://docs.vllm.ai/en/latest/usage/security/#api-key-authentication-limitations) for what it does and does not protect.
...
:   Convert the model using adapters defined in [vllm.model\_executor.models.adapters](../../api/vllm/model_executor/models/adapters/#vllm.model_executor.models.adapters "            vllm.model_executor.models.adapters"). The most common use case is to adapt a text generation model to be used for pooling tasks.
:   Default: `auto`
...
:   User-facing quantization configuration. Carries per-layer-kind specs (linear, moe) and ignore patterns; see :class:[`QuantizationConfigArgs`](../../api/vllm/config/quantization/#vllm.config.quantization.QuantizationConfigArgs "            QuantizationConfigArgs"). Auto-populated from the matching online shorthand when `quantization` is one of the values in `ONLINE_QUANT_SHORTHAND_NAMES`.
...
#### `--return-sampling-mask`, `--no-return-sampling-mask`
...
#### `--enable-trace-replay`, `--no-enable-trace-replay`

:   Whether to allow requests to set `SamplingParams.trace_decode_token_ids`, which forces decoding to follow a predetermined token sequence while still computing real logprobs. Reserved for debugging and RL workflows: enabling it reserves a per-request trace buffer, so it is off by default.
:   Default: `False`
...
:   Pooler config which controls the behaviour of output pooling in pooling models.
    ...
    API docs: [`vllm.config.PoolerConfig`](../../api/vllm/config/#vllm.config.PoolerConfig "            PoolerConfig")
...
#### `--enable-nccl-comm-suspend`, `--no-enable-nccl-comm-suspend`

:   Enable releasing NCCL communicator memory during sleep mode (`ncclCommSuspend`/`ncclCommResume`). Experimental; when disabled (the default) sleep still releases weights/KV-cache memory as before.
:   Default: `False`
...
:   Number of worker threads in the renderer thread pool. The pool is consumed by the async renderer path (e.g. the OpenAI-compatible API server started by `vllm serve`) to parallelize tokenization, chat template rendering, and multimodal preprocessing across concurrent requests.
    ...
    The offline [`LLM`](../../api/vllm/entrypoints/llm/#vllm.entrypoints.llm.LLM "            LLM") entrypoint uses the synchronous renderer path and processes prompts (including multimodal preprocessing) serially, so this setting has no effect there.
:   Default: `1`
...
:   Possible choices: `eager`, [`lazy`](../../api/vllm/logging_utils/lazy/#vllm.logging_utils.lazy.lazy "            lazy"), `prefetch`, `torchao`, `None`
...
:   Communication backend for Decode Context Parallel (DCP). - "ag\_rs": AllGather + ReduceScatter (existing behavior) - "a2a": All-to-All exchange of partial outputs + LSE, then combine with Triton kernel. Reduces NCCL calls from 3 to 2 per layer for MLA models.
    ...
    `None` selects the model default, which is "ag\_rs" unless the model overrides it via [`set_dcp_defaults`](../../api/vllm/config/#vllm.config.ParallelConfig.set_dcp_defaults "            set_dcp_defaults(comm_backend='ag_rs', q_replicate=False)").

#### `--dcp-q-replicate`, `--no-dcp-q-replicate`
...
#### `--enable-batch-sharded-sampling`, `--no-enable-batch-sharded-sampling`
...
:   Expert parallelism configuration.
    ...
    API docs: [`vllm.config.EPLBConfig`](../../api/vllm/config/#vllm.config.EPLBConfig "            EPLBConfig")
...
:   The configurations for fault tolerance.
    ...
    API docs: [`vllm.config.FaultToleranceConfig`](../../api/vllm/config/#vllm.config.FaultToleranceConfig "            FaultToleranceConfig")
...
:   Possible choices: `auto`, `bfloat16`, `float16`, `fp8`, `fp8_ds_mla`, `fp8_e4m3`, `fp8_e5m2`, `fp8_inc`, `fp8_per_token_head`, `int4_per_token_head`, `int8_per_token_head`, `nvfp4`, `nvfp4_4over6`, `nvfp4_ds_mla`, `turboquant_3bit_nc`, `turboquant_4bit_nc`, `turboquant_k3v4_nc`, `turboquant_k8v4`
...
:   If `True`, enables passing multimodal embeddings: for [`LLM`](../../api/vllm/entrypoints/llm/#vllm.entrypoints.llm.LLM "            LLM") class, this refers to tensor inputs under `multi_modal_data`; for the OpenAI-compatible server, this refers to chat messages with content `"type": "*_embeds"`.
...
#### `--mm-device-do-normalize`, `--no-mm-device-do-normalize`
...
:   Possible choices: `detailed`, `none`, `summary`
...
* Top-level Compilation control:
  + [`mode`](../../api/vllm/config/#vllm.config.CompilationConfig.mode "            mode = None

      
          class-attribute
          instance-attribute
      ")
  + [`debug_dump_path`](../../api/vllm/config/#vllm.config.CompilationConfig.debug_dump_path "            debug_dump_path = None

      
          class-attribute
          instance-attribute
      ")
  + [`cache_dir`](../../api/vllm/config/#vllm.config.CompilationConfig.cache_dir "            cache_dir = ''

      
          class-attribute
          instance-attribute
      ")
  + [`backend`](../../api/vllm/config/#vllm.config.CompilationConfig.backend "            backend = ''

      
          class-attribute
          instance-attribute
      ")
  + [`custom_ops`](../../api/vllm/config/#vllm.config.CompilationConfig.custom_ops "            custom_ops = field(default_factory=list)

      
          class-attribute
          instance-attribute
      ")
  + [`splitting_ops`](../../api/vllm/config/#vllm.config.CompilationConfig.splitting_ops "            splitting_ops = None

      
          class-attribute
          instance-attribute
      ")
  + [`compile_mm_encoder`](../../api/vllm/config/#vllm.config.CompilationConfig.compile_mm_encoder "            compile_mm_encoder = False
      ...
      ")
* CudaGraph capture:
  + [`cudagraph_mode`](../../api/vllm/config/#vllm.config.CompilationConfig.cudagraph_mode "            cudagraph_mode = None

      
          class-attribute
          instance-attribute
      ")
  + [`cudagraph_capture_sizes`](../../api/vllm/config/#vllm.config.CompilationConfig.cudagraph_capture_sizes "            cudagraph_capture_sizes = None

      
          class-attribute
          instance-attribute
      ")
  + [`max_cudagraph_capture_size`](../../api/vllm/config/#vllm.config.CompilationConfig.max_cudagraph_capture_size "            max_cudagraph_capture_size = None

      
          class-attribute
          instance-attribute
      ")
  + [`cudagraph_num_of_warmups`](../../api/vllm/config/#vllm.config.CompilationConfig.cudagraph_num_of_warmups "            cudagraph_num_of_warmups = 0

      
          class-attribute
          instance-attribute
      ")
  + [`cudagraph_copy_inputs`](../../api/vllm/config/#vllm.config.CompilationConfig.cudagraph_copy_inputs "            cudagraph_copy_inputs = False
...
* Inductor compilation:
  + [`compile_sizes`](../../api/vllm/config/#vllm.config.CompilationConfig.compile_sizes "            compile_sizes = None
  ...
  + [`inductor_compile_config`](../../api/vllm/config/#vllm.config.CompilationConfig.inductor_compile_config "            inductor_compile_config = field(default_factory=dict)

      
          class-attribute
          instance-attribute
      ")
  + [`inductor_passes`](../../api/vllm/config/#vllm.config.CompilationConfig.inductor_passes "            inductor_passes = field(default_factory=dict)
...
:   Speculative decoding configuration.
    ...
    API docs: [`vllm.config.SpeculativeConfig`](../../api/vllm/config/#vllm.config.SpeculativeConfig "            SpeculativeConfig")

    Should either be a valid JSON string or JSON keys passed individually.

#### `--spec-method`

:   Possible choices: `bailing_hybrid_mtp`, `bailing_hybrid_v3_mtp`, `custom_class`, `deepseek_mtp`, `dflash`, `dots3_note_mtp`, `draft_model`, `dspark`, `eagle`, `eagle3`, `ernie_mtp`, `exaone4_5_mtp`, `exaone_moe_mtp`, `extract_hidden_states`, `gemma4_mtp`, `glm4_moe_lite_mtp`, `glm4_moe_mtp`, `glm_ocr_mtp`, `hy_v3_mtp`, `hy_v4_mtp`, `inkling_mtp`, `kimi_k3_mtp`, `longcat_flash_mtp`, `medusa`, `mimo_mtp`, `mimo_v2_mtp`, `minimax_m3_mtp`, `mlp_speculator`, `mtp`, `nemotron_h_mtp`, `ngram`, `ngram_gpu`, `pangu_ultra_moe_mtp`, `qwen3_5_mtp`, `qwen3_next_mtp`, `qwen4_exp_mtp`, `step3p5_mtp`, `suffix`, `None`
...
:   Diffusion LLM (dLLM) configuration.
    ...
    API docs: [`vllm.config.DiffusionConfig`](../../api/vllm/config/#vllm.config.DiffusionConfig "            DiffusionConfig")
...
:   The configurations for distributed KV cache transfer.
    ...
    API docs: [`vllm.config.KVTransferConfig`](../../api/vllm/config/#vllm.config.KVTransferConfig "            KVTransferConfig")
...
:   The configurations for event publishing.
    ...
    API docs: [`vllm.config.KVEventsConfig`](../../api/vllm/config/#vllm.config.KVEventsConfig "            KVEventsConfig")
...
:   The configurations for distributed EC cache transfer.
    ...
    API docs: [`vllm.config.ECTransferConfig`](../../api/vllm/config/#vllm.config.ECTransferConfig "            ECTransferConfig")
...
:   The configurations for custom encoder cache manager.
    ...
    API docs: [`vllm.config.EncoderCacheManagerConfig`](../../api/vllm/config/#vllm.config.EncoderCacheManagerConfig "            EncoderCacheManagerConfig")
...
:   `torch.compile` and cudagraph capture configuration for the model.
    ...
    API docs: [`vllm.config.CompilationConfig`](../../api/vllm/config/#vllm.config.CompilationConfig "            CompilationConfig")
...
:   Attention configuration.
    ...
    API docs: [`vllm.config.AttentionConfig`](../../api/vllm/config/#vllm.config.AttentionConfig "            AttentionConfig")
...
:   The configurations for reasoning model.
    ...
    API docs: [`vllm.config.ReasoningConfig`](../../api/vllm/config/#vllm.config.ReasoningConfig "            ReasoningConfig")
...
:   Kernel configuration.
    ...
    API docs: [`vllm.config.KernelConfig`](../../api/vllm/config/#vllm.config.KernelConfig "            KernelConfig")
...
:   Structured outputs configuration.
    ...
    API docs: [`vllm.config.StructuredOutputsConfig`](../../api/vllm/config/#vllm.config.StructuredOutputsConfig "            StructuredOutputsConfig")
...
:   Profiling configuration.
    ...
    API docs: [`vllm.config.ProfilerConfig`](../../api/vllm/config/#vllm.config.ProfilerConfig "            ProfilerConfig")
...
:   The configurations for weight transfer during RL training.
    ...
    API docs: [`vllm.config.WeightTransferConfig`](../../api/vllm/config/#vllm.config.WeightTransferConfig "            WeightTransferConfig")
